...
/**
 * This contains various low-level routines for unsigned bigints. The
 * interfaces match the mpn interfaces in gmp, so it should be easy to
 * replace them with fast native functions that are trivial wrappers around
 * the mpn_ functions in gmp (at least on platforms that use 32-bit
 * "limbs").
 */

class MPN {
	/**
	 * Add x[0:size-1] and y, and write the size least significant words of
	 * the result to dest. Return carry, either 0 or 1. All values are
	...
	/**
	 * Add x[0:len-1] and y[0:len-1] and write the len least significant
	 * words of the result to dest[0:len-1]. All words are treated as
	 * unsigned.
	 * 
	 * @return the carry, either 0 or 1 This function is basically the same
	 *         as gmp's mpn_add_n.
	 */
	 ...
	 * significant words of the result to dest[0:size-1]. Return borrow,
	 * either 0 or 1. This is basically the same as gmp's mpn_sub_n
	 * function.
	 */
	...
	public static int sub_n(int[] dest, int[] X, int[] Y, int size) {
		...
		for (int i = 0; i < size; i++) {
			...
			// Invert the high-order bit, because: (unsigned) X >
			// (unsigned) Y
	...
	/**
	 * Multiply x[0:len-1] by y, and write the len least significant words
	 * of the product to dest[0:len-1]. Return the most significant word of
	 * the product. All values are treated as if they were unsigned (i.e.
	 * masked with 0xffffffffL). OK if dest==x (not sure if this is
	 * guaranteed for mpn_mul_1). This function is basically the same as
	 * gmp's mpn_mul_1.
	 */
	 ...
	 * requires that xlen >= ylen. The destination must be distinct from
	 * either input operands. All operands are unsigned. This function is
	 * basically the same gmp's mpn_mul.
	 */

	public static void mul(int[] dest, int[] x, int xlen, int[] y,
			int ylen) {
	 ...
	 * 32)+(unsigned int)(quotient). Assumes (unsigned int)(N>>32) <
	 * (unsigned int)D. Code transcribed from gmp-2.0's mpn_udiv_w_sdiv
	 * function.
	 */
	 ...
	 * 
	 * @return the most significant word of the product, minus borrow-out
	...
	/**
	 * Divide zds[0:nx] by y[0:ny-1]. The remainder ends up in zds[0:ny-1].
	 * The quotient ends up in zds[ny:nx]. Assumes: nx>ny. (int)y[ny-1] < 0
	 * (i.e. most significant bit set)
	 */

	public static void divide(int[] zds, int nx, int[] y, int ny) {
		// This is basically Knuth's formulation of the classical
		// algorithm,
		// but translated from in scm_divbigbig in Jaffar's SCM
		// implementation.
	 ...
	 * For example, for base 10 this is 9, since 10**9 is the largest
	 * number that fits into a words (assuming 32-bit words). This is the
	 * same as gmp's __mp_bases[radix].chars_per_limb.
	 * 
	 * @param radix
	 *            the base
	...
	public static int set_str(int dest[], byte[] str, int str_len,
			int base) {
		int size = 0;
		if ((base & (base - 1)) == 0) {
	...
	/**
	 * Compare {@code x[0:size-1]} with {@code y[0:size-1]}, treating them
	 * as unsigned integers.
	 * 
	 * @return -1, 0, or 1 depending on if {@code x<y}, {@code x==y}, or
	 *         {@code x>y}. This is basically the same as gmp's
	 *         {@code mpn_cmp} function.
	 */
	...
	/**
	 * Compare {@code x[0:xlen-1]} with {@code y[0:ylen-1]}, treating them
	 * as unsigned integers.
	 * 
	 * @return -1, 0, or 1 depending on whether {@code x<y}, {@code x==y},
	 *         or {@code x>y}.
	 */
	...
	/*
	 * Shift x[x_start:x_start+len-1] count bits to the "right" (i.e.
	 * divide by 2**count). Store the len least significant words of the
	 * result at dest. The bits shifted out to the right are returned. OK
	 * if dest==x. Assumes: 0 < count < 32
	 */
	 ...
	 * Shift x[x_start:x_start+len-1] count bits to the "right" (i.e.
	 * divide by 2**count). Store the len least significant words of the
	 * result at dest. OK if dest==x. Assumes: 0 <= count < 32 Same as
	 * rshift, but handles count==0 (and has no return value).
	 */
	 ...
	 * Return the bits shifted out from the most significant digit. Assumes
	 * 0 < count < 32. OK if dest==x.
	 */
	 ...
	 * Assumes both arguments are non-zero. Leaves result in x, and returns
	 * len of result. Also destroys y (actually sets it to a copy of the
	 * result).
	 */
	...
	/**
	 * Calcaulte the Common Lisp "integer-length" function. Assumes input
	 * is canonicalized: len==IntNum.wordsNeeded(words,len)
	 */
	...
	/*
	 * DEBUGGING: public static void dprint (IntNum x) { if (x.words ==
	 * null) System.err.print(Long.toString((long) x.ival & 0xffffffffL,
	 * 16)); else dprint (System.err, x.words, x.ival); } public static
	 * void dprint (int[] x) { dprint (System.err, x, x.length); } public
	 * static void dprint (int[] x, int len) { dprint (System.err, x, len);
	 * } public static void dprint (java.io.PrintStream ps, int[] x, int
	 * len) { ps.print('('); for (int i = 0; i < len; i++) { if (i > 0)
	 * ps.print (' '); ps.print ("#x" + Long.toString ((long) x[i] &
	 * 0xffffffffL, 16)); } ps.print(')'); }
	 */
}
