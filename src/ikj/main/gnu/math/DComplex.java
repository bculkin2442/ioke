 ...
 * 
 * @author Per Bothner
 * @author Some algorithms were transcribed from GNU libstdc++, written by
 *         Jason Merrill. Also see below for copyrights for functions taken
 *         from fdlib and f2c.
 */
...
public class DComplex extends Complex implements Externalizable {
	...
	public boolean equals(Object obj) {
		...
		return y.unit() == Unit.Empty
				&& (Double.doubleToLongBits(real) == Double
						.doubleToLongBits(y.reValue()))
				&& (Double.doubleToLongBits(imag) == Double
	...
	public static DComplex power(double x_re, double x_im, double y_re,
			double y_im) {
		double h;
		/* #ifdef JAVA5 */
	...
	/****************************************************************
	 * Copyright 1990, 1991, 1992, 1993 by AT&T Bell Laboratories and
	 * Bellcore.
	 * 
	 ...
	 * names of AT&T Bell Laboratories or Bellcore or any of their entities
	...
	public static DComplex div(double x_re, double x_im, double y_re,
			double y_im) {
	...
	/*
	 * ==================================================== Copyright (C)
	 * 1993 by Sun Microsystems, Inc. All rights reserved.
	 *
	 ...
	 *
	 * So, compute sqrt(x*x+y*y) with some care as follows to get the error
	 * below 1 ulp:
	 *
	 * Assume x>y>0; (if possible, set rounding to round-to-nearest) 1. if
	 * x > 2y use x1*x1+(y*y+(x2*(x+x1))) for x*x+y*y where x1 = x with
	 * lower 32 bits cleared, x2 = x-x1; else 2. if x <= 2y use
	 * t1*y1+((x-y)*(x-y)+(t1*y2+t2*y)) where t1 = 2x with lower 32 bits
	 * cleared, t2 = 2x-t1, y1= y with lower 32 bits chopped, y2 = y-y1.
	 *
	 * NOTE: scaling may be necessary if some argument is too large or too
	 * tiny
	 *
	 * Special cases: hypot(x,y) is INF if x or y is +INF or -INF; else
	 * hypot(x,y) is NAN if x or y is NAN.
	 *
	 * Accuracy: hypot(x,y) returns sqrt(x^2+y^2) with error less than 1
	...
	static double hypot(double x, double y) {
		...
		/*
		 * Now a is max (abs(x), abs(y)) and b is min(abs(x), abs(y)); la
		...
		w = Math.sqrt(w);
		if (k != 0) { // t1 = 2^k
			t1 = Double.longBitsToDouble(
					0x3ff0000000000000L + ((long) k << 52));
			w *= t1;
		}
		return w;
	}
