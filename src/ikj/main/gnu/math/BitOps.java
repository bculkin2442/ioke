 ...
 * There are no BitOps object - all the functions here are static. The
 * semantics used are the same as for Common Lisp.
 * 
 * @author Per Bothner
 */
...
public class BitOps {
	...
	public static IntNum extract(IntNum x, int startBit, int endBit) {
		// System.err.print("extract(["); if (x.words!=null)
		// MPN.dprint(x.words);
		// System.err.println (","+x.ival+"], start:"+startBit+",
		// end:"+endBit);
		if (endBit < 32) {
		...
		// Allocate a work buffer, which has to be large enough for the
		// result
	...
	static final byte[] bit4_count = { 0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3,
			2, 3, 3, 4 };
	...
	/**
	 * Count one bits in an IntNum. If argument is negative, count zero
	 * bits instead.
	 */
