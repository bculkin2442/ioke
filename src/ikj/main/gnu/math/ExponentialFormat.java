...
/**
 * Format a real number using a floating-point format. However, if
 * `general' is true, and the number "fits", use a fixed-point format (like
 * printf %g). Used for Common Lisp specs ~E and ~G; also C-style %e and
 * %g.
 */

public class ExponentialFormat extends java.text.Format {
	/**
	 * Number of fractional digits to show. This is `d' in the CommonLisp
	 * spec.
	 */
	public int			fracDigits		= -1;

	/**
	 * Number of digits to show in the integer part of the result. If
	 * positive, The number of digits before the decimal point. If
	 * negative, the -intDigits zeros are emitted after the decimal point.
	 * This is `k' in the CommonLisp spec.
	 */
	public int			intDigits;

	/**
	 * Number of digits to show in the exponent. Zero means unspecified -
	 * show as many as needed.
	 */
	...
	public StringBuffer format(float value, StringBuffer sbuf,
			FieldPosition fpos) {
		return format(value, fracDigits < 0 ? Float.toString(value) : null,
				sbuf, fpos);
	}

	public StringBuffer format(double value, StringBuffer sbuf,
			FieldPosition fpos) {
		return format(value,
				fracDigits < 0 ? Double.toString(value) : null, sbuf,
				fpos);
	}

	StringBuffer format(double value, String dstr, StringBuffer sbuf,
			FieldPosition fpos) {
		int k = intDigits;
		int d = fracDigits;
		...
		if (fracUnspecified
				&& (dot + 1 == sbuf.length()
						|| sbuf.charAt(dot + 1) == exponentChar)
				&& (width <= 0 || i > 0)) {
			i--;
			sbuf.insert(dot + 1, '0');
		}
		...
		if ((i >= 0 || width <= 0)
			...
			if (!showExponent
					// The CommonLisp spec requires adding spaces on the
					// right
					// when a ~g format ends up using fixed-point format,
					// corresponding
					// to the space otherwise used for the exponent.
					// However, it seems
