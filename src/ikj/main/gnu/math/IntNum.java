...
public class IntNum extends RatNum implements Externalizable {
	/**
	 * All integers are stored in 2's-complement form. If words == null,
	 * the ival is the value of this IntNum. Otherwise, the first ival
	 * elements of words make the value of this IntNum, stored in
	 * little-endian order, 2's-complement form.
	 */
	public int				ival;
	public int[]			words;
	...
	public static IntNum make(int value) {
		if (value >= minFixNum && value <= maxFixNum) {
			return smallFixNums[value - minFixNum];
		}
	...
	/**
	 * Make a canonicalized IntNum from an array of words. The array may be
	...
	/**
	 * Change words.length to nwords. We allow words.length to be upto
	 * nwords+2 without reallocating.
	 */
	...
	/**
	 * Calculate how many words are significant in words[0:len-1]. Returns
	 * the least value x such that x>0 && words[0:x-1]==words[0:len-1],
	...
	/**
	 * Destructively set the value of this to the given words. The words
	 * array is reused, not copied.
	 */
	...
	public static IntNum add(IntNum x, IntNum y, int k) {
		if (x.words == null && y.words == null) {
			return IntNum.make((long) k * (long) y.ival + x.ival);
		}
		if (k != 1) {
			if (k == -1) {
				y = IntNum.neg(y);
	...
	public static void divide(long x, long y, IntNum quotient,
			IntNum remainder, int rounding_mode) {
		boolean xNegative, yNegative;
		if (x < 0) {
			xNegative = true;
			if (x == Long.MIN_VALUE) {
				divide(IntNum.make(x), IntNum.make(y), quotient, remainder,
						rounding_mode);
				return;
			}
			x = -x;
		...
		if (y < 0) {
			yNegative = true;
			if (y == Long.MIN_VALUE) {
				if (rounding_mode == TRUNCATE) { // x != Long.Min_VALUE
													// implies abs(x) <
													// abs(y)
					if (quotient != null) {
						quotient.set(0);
					...
					divide(IntNum.make(x), IntNum.make(y), quotient,
							remainder, rounding_mode);
				}
				return;
			}
			y = -y;
	 ...
	 * @param remainder
	 *            is set to the remainder of the result (iff
	 *            remainder!=null)
	 * @param rounding_mode
	 *            one of FLOOR, CEILING, TRUNCATE, or ROUND.
	 */
	public static void divide(IntNum x, IntNum y, IntNum quotient,
			IntNum remainder, int rounding_mode) {
		...
		{
			// Normalize the denominator, i.e. make its most significant
			// bit set by
			// shifting it normalization_steps bits to the left. Also shift
			// the
			...
			if (nshift != 0) {
				// Shift up the denominator setting the most significant
				// bit of
		...
		boolean add_one = false;
		if (rlen > 1 || ywords[0] != 0) { // Non-zero remainder i.e.
											// in-exact quotient.
			switch (rounding_mode) {
				case TRUNCATE:
					break;
				...
				case ROUND:
					...
					IntNum tmp = remainder == null ? new IntNum()
							: remainder;
					tmp.set(ywords, rlen);
					tmp = shift(tmp, 1);
					if (yNegative) {
		...
		if (remainder != null) {
			...
			if (add_one) {
				// Subtract the remainder from Y:
				// abs(R) = abs(Y) - abs(orig_rem) = -(abs(orig_rem) -
				// abs(Y)).
				IntNum tmp;
				if (y.words == null) {
					tmp = remainder;
					tmp.set(yNegative ? ywords[0] + y.ival
							: ywords[0] - y.ival);
				} else {
	 ...
	 * 
	 * @param x
	 *            the value (base) to exponentiate
	 * @param y
	...
	public static IntNum shift(IntNum x, int count) {
		if (x.words == null) {
			if (count <= 0) {
				return make(count > -32 ? x.ival >> (-count)
						: x.ival < 0 ? -1 : 0);
			}
			if (count < 32) {
				...
				for (int i = len; --i >= 0;) {
					...
					for (int j = 8; --j >= 0;) {
						...
						if (hex_digit > 0 || buffer.length() > buf_start) {
							buffer.append(
									Character.forDigit(hex_digit, 16));
						}
					}
				}
			} else {
	...
	public static IntNum valueOf(String s, int radix)
		...
		int len = s.length();
		// Testing (len < 2 * MPN.chars_per_word(radix)) would be more
		// accurate,
	...
	/**
	 * Convert a semi-processed IntNum to double. Number must be
	 * non-negative. Multiplies by a power of two, applies sign, and
	 * converts to double, with the usual java rounding.
	 * 
	 * @param exp
	 ...
	 *            true if negative
	 * @param remainder
	 *            true if the IntNum is the result of a truncating division
	 *            that had non-zero remainder. To ensure proper rounding in
	...
	public double roundToDouble(int exp, boolean neg, boolean remainder) {
		...
		// leading one. This is stored excess 1023 in the exponent bit
		// field.
		exp += il - 1;
		...
		// Normal round-to-even rule: round up if the bit dropped is a one,
		// and
	...
	/**
	 * Calculates {@code ceiling(log2(this < 0 ? -this : this+1))}. See
	 * Common Lisp: the Language, 2nd ed, p. 361.
	 */
	...
	/**
	 * @serialData If the value is in the range (int)0xC000000 ..
	 *             0x7fffffff (inclusive) write out the value (using
	 *             writeInt). Otherwise, write (using writeInt)
	 *             (0x80000000|nwords), where nwords is the number of words
	 *             following. The words are the minimal 2's complement
	 *             big-endian representation of the value, written using
	 *             writeint. (Even if the current value is not
	 *             canonicalized, the output is).
	 */
	@Override
	public void writeExternal(ObjectOutput out) throws IOException {
		int nwords = words == null ? 1 : wordsNeeded(words, ival);
		if (nwords <= 1) {
			int i = words == null ? ival
					: words.length == 0 ? 0 : words[0];
			if (i >= 0xC0000000) {
				out.writeInt(i);
			} else {
