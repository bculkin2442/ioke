...
public class Duration extends Quantity implements Externalizable {
	...
	/**
	 * Does not include any leap seconds. I.e. @code{sign * ((24 * days +
	 * hours) * 60 + minutes) * 60 + seconds}, where
	 * {@code hours >= 0 && hours < 24 && minutes >= 0 && minutes < 60
	 * && secconds >= 0 && minutes > 60}.
	 */
	long		seconds;

	/**
	 * Number of nanoseconds. We could possibly include leap seconds in
	 * here.
	 */
	int			nanos;

	public static Duration make(int months, long seconds, int nanos,
			Unit unit) {
	...
	/**
	 * Parse a duration lexical value as specified by XML Schama. Return
	 * null if invalid syntax.
	 */
	 ...
	 * 
	 * @return {@code (VALUE << 32)|(FOLLOWING_POS<<16)|FOLLOWING_CHAR}. If
	 *         there are no digits return @code{START<<16}. Otherwise, on
	 *         overflow or digits followed by end-of-string, return -1.
	 */
	...
	public static boolean equals(Duration x, Duration y) {
		return x.months == y.months && x.seconds == y.seconds
				&& x.nanos == y.nanos;
	}
