...
/* #endif */
{
	...
	/**
	 * Convert to an exact number. Implements the Scheme inexact->exact
	 * (for real numbers).
	 */
	 ...
	 * mode. Note an inexact argument gives an inexact result, following
	 * Scheme. See also RatNum.toExactInt.
	 */
	...
	/*
	 * public static String toScaledIntString (double f, int k) { switch
	 * (k) { case 0: break; case 1: f = f * 10; break; case 2: f = f * 100;
	 * break; case 3: f = f * 1000; break; default: return toScaledInt(f,
	 * k).toString(); } return Long.toString((long) f); }
	 */

	/**
	 * Implements the Comparable interface. This ordering isn't fully
	...
	/**
	 * Convert result of Double.toString or Float.toString to scientific
	 * notation. Does not validate the input.
	 */
	...
	public static int toStringScientific(String dstr, StringBuffer sbuf) {
		...
		int exp;
		int len = dstr.length();
		if (dstr.charAt(pos) == '0') { // Value is < 1.0.
			int start = pos;
			for (;;) {
				if (pos == len) {
					sbuf.append("0");
			...
			// We want ndigits-1 fractional digits. Hence we need to move
			// the
			// decimal point ndigits-1-(len-dot-1) == ndigits-len+dot
			// positions
			// to the left. This becomes the exponent we need.
			exp = ndigits - len + dot;
			sbuf.append(dstr.charAt(pos++)); // Copy initial digit before
												// point.
			sbuf.append('.');
			while (pos < len) {
				char ch = dstr.charAt(pos++);
	...
	public static String toStringDecimal(String dstr) {
		...
		if (dstr.charAt(indexE + 1) != '-') {
			throw new Error(
					"not implemented: toStringDecimal given non-negative exponent: "
							+ dstr);
		} else {
