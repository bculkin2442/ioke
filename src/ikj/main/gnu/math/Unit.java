...
public abstract class Unit extends Quantity {
	...
	/**
	 * A Unit equivalent to this unit, divided by factor. Same as the value
	 * of the dimensions() only.
	 */
	Unit				base;
	...
	/*
	 * public String toString (Complex val) { String str =
	 * toString(val.re()); RealNum im = val.im(); if (im.isZero()) return
	 * str; // This conflicts with using '@' for polar notation. return str
	 * + "@" + toString(im); }
	 */
	...
	/**
	 * A magic factor to indicate units that have the same "dimension" but
	 * not a fixed multiple. E.g. "month" and "day", or money of different
	 * currencies. Since they have the same dimension, they can be added to
	 * get an (unimplemented) combined quantity, but they cannot be
	 * compared. No general support yet, but used for time Duration.
	 */
	public static double			NON_COMBINABLE	= 0.0;

	public static final BaseUnit	meter			= new BaseUnit("m",
			"Length");
	public static final BaseUnit	duration		= new BaseUnit(
			"duration", "Time");
	public static final BaseUnit	gram			= new BaseUnit("g",
			"Mass");
	public static final Unit		cm				= define("cm", 0.01,
			meter);
	public static final Unit		mm				= define("mm", 0.1,
			cm);
	public static final Unit		in				= define("in", 0.0254,
			meter);
	public static final Unit		pt				= define("pt",
			0.0003527778, meter);
	public static final Unit		pica			= define("pica",
			0.004233333, meter);
	public static final Unit		radian			= define("rad", 1.0,
			Unit.Empty);

	public static final NamedUnit	date			= new NamedUnit("date",
			NON_COMBINABLE, duration);
	public static final NamedUnit	second			= new NamedUnit("s",
			NON_COMBINABLE, duration);
	public static final NamedUnit	month			= new NamedUnit(
			"month", NON_COMBINABLE, duration);
	public static final Unit		minute			= define("min", 60.0,
			second);
	public static final Unit		hour			= define("hour", 60.0,
			minute);
}
