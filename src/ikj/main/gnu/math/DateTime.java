...
// This is free software; for terms and warranty disclaimer see
// ../../COPYING.
...
/**
 * Represents a date and/or time. Similar functionality as
 * java.util.Calendar (and uses GregorianCalendar internally) but supports
 * arithmetic. Can be for XML Schema date/time types, specifically as used
 * in XPath/Xquery..
 */
...
public class DateTime extends Quantity implements Cloneable {
	...
	/*
	 * static final int REFERENCE_YEAR = 1972; static final int
	 * REFERENCE_MONTH = 0; // January static final int REFERENCE_DAY = 0;
	 * // January 1
	 */
	...
	public static final int	DATE_MASK			= YEAR_MASK | MONTH_MASK
			| DAY_MASK;
	public static final int	TIME_MASK			= HOURS_MASK | MINUTES_MASK
			| SECONDS_MASK;

	public int components() {
		return mask & ~TIMEZONE_MASK;
	}
	...
	int parseTime(String str, int start) {
		...
		int hour = part >> 16;
		pos = part & 0xffff;
		if (hour <= 24 && pos == start + 2 && pos != len
				&& str.charAt(pos) == ':') {
			start = pos + 1;
			part = parseDigits(str, start);
			int minute = part >> 16;
			pos = part & 0xffff;
			if (minute < 60 && pos == start + 2 && pos != len
					&& str.charAt(pos) == ':') {
				start = pos + 1;
				part = parseDigits(str, start);
				int second = part >> 16;
				...
				if (second < 60 && pos == start + 2) {
					if (pos + 1 < len && str.charAt(pos) == '.'
							&& Character.digit(str.charAt(pos + 1),
									10) >= 0) {
						start = pos + 1;
						pos = start;
						int nanos = 0;
	...
	/*
	 * public Object getSecondsObject () { return
	 * IntNum.make(getWholeSeconds()); }
	 */
	...
	private static void append(int value, StringBuffer sbuf,
			int minWidth) {
