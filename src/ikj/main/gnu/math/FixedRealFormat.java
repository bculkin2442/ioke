...
/**
 * Format a real number using a fixed-point format. Used for Common Lisp
 * specs ~F and ~$; also C-style %f.
 */
...
public class FixedRealFormat extends java.text.Format {
	...
	public void format(RatNum number, boolean negative, StringBuffer sbuf,
			FieldPosition fpos) {
		...
		if (decimals < 0) {
			...
			if (decimals == 0
					&& (width <= 0 || signLen + digits + 1 < width)) {
				decimals = 1;
				length++;
			...
			string = RealNum.toScaledInt(number, decimals + scale)
					.toString();
			sbuf.append(string);
		...
		/*
		 * Requires JDK1.2 FieldPosition extensions: if (fpos == null) {
		 * newSize++; if (fpos.getField() == FRACTION_FIELD) {
		 * fpos.setBeginIndex(newSize-decimals); fpos.setEndIndex(newSize);
