...
public abstract class Complex extends Quantity {
	...
	public Numeric abs() {
		...
		/* #else */
		return new DFloNum(
				DComplex.hypot(doubleRealValue(), doubleImagValue()));
		/* #endif */
	}

	public RealNum angle() {
		return new DFloNum(
				Math.atan2(doubleImagValue(), doubleRealValue()));
	}
	...
	/*
	 * Unit unit () { return Unit.Empty; } Dimesions dims() { return
	 * unit().dims; }
	 */
	...
	public static Complex times(Complex x, Complex y) {
		...
		RealNum y_im = y.im();
		return Complex.make(
				RealNum.add(RealNum.times(x_re, y_re),
