...
/**
 * A Dimensions object represents the product or ratio of BaseUnits. The
 * idea is that in order to add two Quantities (such as 3mm + 5cm) their
 * Dimensions have to match. Equal dimensions are also ==.
 * 
 * @author Per Bothner.
 */
...
public class Dimensions {
	/**
	 * The BaseUnits that this dimension is defined in terms of. The
	 * BaseUnits are in order of their index, and the last element is
	 * Unit.Empty.
	 */
	BaseUnit[]					bases;

	/**
	 * For each baseunit in bases[i], powers[i] is the corresponding
	 * exponent. It is never zero (as long as i is less than the index of
	 * Unit.Empty).
	 */
	short[]						powers;
	...
	private boolean matchesProduct(Dimensions a, int mul_a, Dimensions b,
			int mul_b) {
		int a_i = 0, b_i = 0;
		for (int t_i = 0;;) {
			BaseUnit a_base = a.bases[a_i];
	...
	public static Dimensions product(Dimensions a, int mul_a, Dimensions b,
			int mul_b) {
