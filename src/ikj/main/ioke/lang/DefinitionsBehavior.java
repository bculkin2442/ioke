...
public class DefinitionsBehavior {
	public static void init(IokeObject obj) throws ControlFlow {
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("method") {
					...
					public Object activate(IokeObject method,
						...
						if (args.size() == 0) {
							final Message mx = new Message(context.runtime,
									"nil", null, false);
							mx.setFile(Message.file(message));
							mx.setLine(Message.line(message));
							mx.setPosition(Message.position(message));
							final IokeObject mmx = context.runtime
									.createMessage(mx);
							return runtime
									.newMethod(null, runtime.defaultMethod,
											new DefaultMethod(context,
						...
						int start = 0;
						if (args.size() > 1
								&& ((IokeObject) Message.getArg1(message))
										.getName().equals(
												"internal:createText")) {
							start++;
							String s = ((String) ((IokeObject) args.get(0))
									.getArguments().get(0));
							doc = s;
						}

						ArgumentsDefinition def = DefaultArgumentsDefinition
								.createFrom(args, start, args.size() - 1,
										message, on, context);

						return runtime.newMethod(doc,
								runtime.defaultMethod,
								new DefaultMethod(context, def,
										(IokeObject) args
												.get(args.size() - 1)));
					}
				}));
				...
				new NativeMethod("macro") {
					...
					public Object activate(IokeObject method,
						...
						if (args.size() == 0) {
							...
							return runtime.newMacro(null,
									runtime.defaultMacro,
									new DefaultMacro(context, mmx));
						}
						...
						return runtime.newMacro(doc, runtime.defaultMacro,
								new DefaultMacro(context,
										(IokeObject) args.get(start)));
					}
				}));
				...
				new NativeMethod("syntax") {
					...
					public Object activate(IokeObject method,
						...
						if (args.size() == 0) {
							...
							return runtime.newMacro(null,
									runtime.defaultSyntax,
									new DefaultSyntax(context, mmx));
						}
						...
						return runtime.newMacro(doc, runtime.defaultSyntax,
								new DefaultSyntax(context,
										(IokeObject) args.get(start)));
					}
				}));
				...
				new NativeMethod("lecro") {
					...
					public Object activate(IokeObject method,
						...
						if (args.size() == 0) {
							...
							return runtime.newMacro(null,
									runtime.lexicalMacro,
									new LexicalMacro(context, mmx));
						}
						...
						return runtime.newMacro(doc, runtime.lexicalMacro,
								new LexicalMacro(context,
										(IokeObject) args.get(start)));
					}
				}));
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("fn") {
					...
					public Object activate(IokeObject method,
						...
						List<Object> args = message.getArguments();
						if (args.isEmpty()) {
							return runtime.newLexicalBlock(null,
									runtime.lexicalBlock,
									new LexicalBlock(context,
											DefaultArgumentsDefinition
													.empty(),
											method.runtime.nilMessage));
						}
						...
						IokeObject code = IokeObject
								.as(args.get(args.size() - 1), context);

						ArgumentsDefinition def = DefaultArgumentsDefinition
								.createFrom(args, start, args.size() - 1,
										message, on, context);
						return runtime.newLexicalBlock(doc,
								runtime.lexicalBlock,
								new LexicalBlock(context, def, code));
					}
				}));
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("aliasMethod") {
					private final DefaultArgumentsDefinition ARGUMENTS = DefaultArgumentsDefinition
							.builder().withRequiredPositional("oldName")
					...
					public Object activate(IokeObject method,
						...
						String fromName = Text.getText(Interpreter.send(
								runtime.asText, context, args.get(0)));
						String toName = Text.getText(Interpreter.send(
								runtime.asText, context, args.get(1)));
						IokeObject.as(on, context).aliasMethod(fromName,
								toName, message, context);
						return on;
					}
				}));
	}
