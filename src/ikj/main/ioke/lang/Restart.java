...
public class Restart {
	public static void init(IokeObject restart) throws ControlFlow {
		...
		restart.registerCell("report",
				runtime.evaluateString("fn(r, \"restart: \" + r name)",
						runtime.message, runtime.ground));
		restart.registerCell("test", runtime.evaluateString("fn(c, true)",
				runtime.message, runtime.ground));
		restart.registerCell("code", runtime.evaluateString("fn()",
				runtime.message, runtime.ground));
		restart.registerCell("argumentNames",
				runtime.evaluateString("method(self code argumentNames)",
						runtime.message, runtime.ground));
	}
	...
	public abstract static class JavaRestart {
		...
		public abstract IokeObject invoke(IokeObject context,
				List<Object> arguments) throws ControlFlow;
	}

	public abstract static class ArgumentGivingRestart
			extends JavaRestart {
	...
	public static class DefaultValuesGivingRestart extends JavaRestart {
		...
		public DefaultValuesGivingRestart(String name, IokeObject value,
				int repeat) {
			this.name = name;
			this.value = value;
			this.repeat = repeat;
