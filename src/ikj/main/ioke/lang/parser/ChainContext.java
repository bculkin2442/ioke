...
public final class ChainContext {
	...
	BufferedChain		chains			= new BufferedChain(null, null,
			null);;

	public IokeObject	last			= null;
	IokeObject			head			= null;

	public Level		currentLevel	= new Level(-1, null, null,
			Level.Type.REGULAR);
	...
	public IokeObject prepareAssignmentMessage() throws ControlFlow {
		if (chains.last != null
				&& chains.last == currentLevel.operatorMessage) {
			if (currentLevel.type == Level.Type.ASSIGNMENT
					&& head == null) {
				...
				return assgn;
					...
					&& currentLevel.type != Level.Type.ASSIGNMENT) {
				pop();
				currentLevel = currentLevel.parent;
			}
		}
	...
	public IokeObject pop() throws ControlFlow {
		if (head != null) {
			while (Message.isTerminator(head)
					&& Message.next(head) != null) {
	...
	public void popOperatorsTo(int precedence) throws ControlFlow {
		while ((currentLevel.precedence != -1
				|| currentLevel.type == Level.Type.UNARY)
				&& currentLevel.precedence <= precedence) {
			IokeObject arg = pop();
			if (arg != null && Message.isTerminator(arg)
					&& Message.next(arg) == null) {
				arg = null;
			}

			IokeObject op = currentLevel.operatorMessage;
			if (currentLevel.type == Level.Type.INVERTED
					&& Message.prev(op) != null) {
