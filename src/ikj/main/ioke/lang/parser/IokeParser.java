...
public class IokeParser {
	...
	public IokeParser(Runtime runtime, Reader reader, IokeObject context,
	...
	protected void fail(int l, int c, String message, String expected,
			String got) throws ControlFlow {
		String file = ((IokeSystem) IokeObject.data(runtime.system))
				.currentFile();

		final IokeObject condition = IokeObject.as(
				IokeObject.getCellChain(runtime.condition, this.message,
						this.context, "Error", "Parser", "Syntax"),
				this.context).mimic(this.message, this.context);
		...
		condition.setCell("text", runtime
				.newText(file + ":" + l + ":" + c + ": " + message));
	...
	protected void parseCharacter(int c) throws IOException, ControlFlow {
		...
		if (rr != c) {
			fail(l, cc,
					"Expected: '" + (char) c + "' got: " + charDesc(rr),
					"" + (char) c, charDesc(rr));
		}
	}

	protected boolean isUnary(String name) {
		return unaryOperators.contains(name)
				&& (top.head == null || Message.isTerminator(top.last));
	}
