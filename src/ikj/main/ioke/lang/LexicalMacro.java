 ...
 */
public class LexicalMacro extends IokeData
		implements AssociatedCode, Named, Inspectable {
	String				name;
				...
				new NativeMethod.WithNoArguments("name") {
					...
					public Object activate(IokeObject self,
						...
						return context.runtime.newText(
								((LexicalMacro) IokeObject.data(on)).name);
					}
				}));
		obj.registerMethod(obj.runtime.newNativeMethod(
				...
				new NativeMethod.WithNoArguments("inspect") {
					...
					public Object activate(IokeObject self,
						...
						return context.runtime
								.newText(LexicalMacro.getInspect(on));
					}
				}));
		obj.registerMethod(obj.runtime.newNativeMethod(
				...
				new NativeMethod.WithNoArguments("notice") {
					...
					public Object activate(IokeObject self,
						...
						return context.runtime
								.newText(LexicalMacro.getNotice(on));
					}
				}));
		obj.registerMethod(obj.runtime.newNativeMethod(
		...
		if (lm.code == null) {
			IokeObject condition = IokeObject
					.as(IokeObject.getCellChain(
							dynamicContext.runtime.condition, message,
							dynamicContext, "Error", "Invocation",
							"NotActivatable"), dynamicContext)
					.mimic(message, dynamicContext);
			condition.setCell("message", message);
			condition.setCell("context", dynamicContext);
			condition.setCell("receiver", on);
			condition.setCell("method", self);
			condition.setCell("report", dynamicContext.runtime.newText(
					"You tried to activate a method without any code - did you by any chance activate the LexicalMacro kind by referring to it without wrapping it inside a call to cell?"));
			dynamicContext.runtime.errorCondition(condition);
			return null;
		}

		IokeObject c = self.runtime.newLexicalContext(on,
				"Lexical macro activation context", lm.context);
	...
	public static Object activateFixed(IokeObject self,
		...
		c.setCell("outerScope", lm.context);
		c.setCell("call", dynamicContext.runtime.newCallFrom(c, message,
				dynamicContext, IokeObject.as(on, dynamicContext)));
