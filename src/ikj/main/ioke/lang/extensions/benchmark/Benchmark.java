...
public abstract class Benchmark {
	...
	public static IokeObject create(Runtime runtime) throws ControlFlow {
		IokeObject bm = new IokeObject(runtime,
				"Benchmark is a module that makes it easy to test the time code takes to run");
		Benchmark.init(bm);
		return bm;
	}
	...
	public static void init(IokeObject bm) throws ControlFlow {
		...
		bm.registerMethod(runtime.newNativeMethod(
				"expects two optional numbers, x (default 10) and y (default 1), and a block of code to run, and will run benchmark this block x times, while looping y times in each benchmark. after each loop will print the timings for this loop",
				new NativeMethod("report") {
					...
					public Object activate(IokeObject method,
						...
						int index = 0;
						if (count > 1) {
							bmRounds = ((Number) IokeObject.data(
									IokeObject.convertToNumber(Interpreter
							...
							index++;
							if (count > 2) {
								iterations = ((Number) IokeObject
										.data(IokeObject.convertToNumber(
												Interpreter
						...
						for (int i = 0; i < bmRounds; i++) {
							...
							String theCode = Message
									.thisCode(((IokeObject) message
											.getArguments().get(index)));

							Interpreter.send(
									context.runtime.printlnMessage,
									context,
									Interpreter.send(
											context.runtime.outMessage,
											context,
											context.runtime.system),
									context.runtime.newText(String.format(
											"%-32.32s %.6s.%09d", theCode,
											secs, rest)));
						}
