...
public class ConditionsBehavior {
	public static IokeObject signal(Object datum,
			List<Object> positionalArgs, Map<String, Object> keywordArgs,
			IokeObject message, IokeObject context) throws ControlFlow {
		IokeObject newCondition = null;
		if (Text.isText(datum)) {
			newCondition = IokeObject.as(context.runtime.condition
					.getCell(message, context, "Default"), context)
			...
			if (keywordArgs.size() == 0) {
				...
				newCondition = IokeObject.as(datum, context).mimic(message,
						context);
				newCondition.setCell("context", context);
				for (Map.Entry<String, Object> val : keywordArgs
						.entrySet()) {
					String s = val.getKey();
					newCondition.setCell(s.substring(0, s.length() - 1),
							val.getValue());
				}
			}
		}

		Runtime.RescueInfo rescue = context.runtime
				.findActiveRescueFor(newCondition);

		List<Runtime.HandlerInfo> handlers = context.runtime
				.findActiveHandlersFor(newCondition, (rescue == null)
						? new Runtime.BindIndex(-1, -1) : rescue.index);

		for (Runtime.HandlerInfo rhi : handlers) {
			Interpreter.send(context.runtime.callMessage, context,
					Interpreter.send(context.runtime.handlerMessage,
							context, rhi.handler),
					newCondition);
		}
	...
	public static void init(IokeObject obj) throws ControlFlow {
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("restart") {
					private final DefaultArgumentsDefinition ARGUMENTS = DefaultArgumentsDefinition
							.builder()
							.withOptionalPositionalUnevaluated("name")
							.withKeyword("report").withKeyword("test")
					...
					public Object activate(IokeObject method,
						...
						if (argCount > 4) {
							final IokeObject condition = IokeObject
									.as(IokeObject.getCellChain(
											...
											context, "Error", "Invocation",
											"TooManyArguments"), context)
									.mimic(message, context);
							condition.setCell("message", message);
							condition.setCell("context", context);
							condition.setCell("receiver", on);
							condition.setCell("extra", runtime
									.newList(args.subList(4, argCount)));
							...
							final IokeObject condition = IokeObject
									.as(IokeObject.getCellChain(
											runtime.condition, message,
											context, "Error", "Invocation",
											"TooFewArguments"), context)
									.mimic(message, context);
							condition.setCell("message", message);
							condition.setCell("context", context);
							condition.setCell("receiver", on);
							condition.setCell("missing",
									runtime.newNumber(1 - argCount));
						...
						code = IokeObject.as(
								runtime.interpreter.evaluate(code, context,
										context.getRealContext(), context),
								context);
						Object restart = Interpreter.send(runtime.mimic,
								context, runtime.restart);

						IokeObject.setCell(restart, "code", code, context);

						if (null != name) {
							IokeObject.setCell(restart, "name",
									runtime.getSymbol(name), context);
						}
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("rescue") {
					private final DefaultArgumentsDefinition ARGUMENTS = DefaultArgumentsDefinition
							.builder().withRest("conditionsAndAction")
							.getArguments();
					...
					public Object activate(IokeObject method,
						...
						Object handler = Interpreter.getEvaluatedArgument(
								message, count - 1, context);
						Object rescue = Interpreter.send(
								context.runtime.mimic, context,
								context.runtime.rescue);

						IokeObject.setCell(rescue, "handler", handler,
								context);
						IokeObject.setCell(rescue, "conditions",
								context.runtime.newList(conds), context);
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("handle") {
					...
					public Object activate(IokeObject method,
						...
						Object code = Interpreter.getEvaluatedArgument(
								message, count - 1, context);
						Object handle = Interpreter.send(
								context.runtime.mimic, context,
								context.runtime.handler);

						IokeObject.setCell(handle, "handler", code,
								context);
						IokeObject.setCell(handle, "conditions",
								context.runtime.newList(conds), context);
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("bind") {
					...
					public Object activate(IokeObject method,
						...
						try {
							for (Object o : args.subList(0,
									argCount - 1)) {
								IokeObject msg = IokeObject.as(o, context);
								IokeObject bindable = IokeObject.as(
										runtime.interpreter.evaluate(msg,
												context,
												context.getRealContext(),
												context),
										context);
								...
								do {
									loop = false;
									if (IokeObject.isKind(bindable,
											"Restart")) {
										Object ioName = Interpreter.send(
										...
										restarts.add(0,
												new Runtime.RestartInfo(
														name, bindable,
														restarts, index,
														null));
										index = index.nextCol();
									} else if (IokeObject.isKind(bindable,
											"Rescue")) {
										Object conditions = Interpreter
												.send(runtime.conditionsMessage,
														context, bindable);
										List<Object> applicable = IokeList
												.getList(conditions);
										rescues.add(0,
												new Runtime.RescueInfo(
														bindable,
														applicable,
														rescues, index));
										index = index.nextCol();
										...
										final IokeObject condition = IokeObject
												.as(IokeObject
														...
														context)
												.mimic(message, context);
										condition.setCell("message",
												message);
										condition.setCell("context",
												context);
										condition.setCell("receiver", on);
							...
							return runtime.interpreter.evaluate(code,
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("invokeRestart") {
					private final DefaultArgumentsDefinition ARGUMENTS = DefaultArgumentsDefinition
							.builder()
				...
				new NativeMethod("findRestart") {
					...
					public Object activate(IokeObject method,
						...
						IokeObject restart = IokeObject.as(args.get(0),
								context);
						Runtime.RestartInfo realRestart = null;
						while (!(restart.isSymbol()
								|| restart.getKind(message, context)
										.equals("Restart"))) {
							final IokeObject condition = IokeObject
									.as(IokeObject.getCellChain(
											runtime.condition, message,
						...
						} else if (restart.getKind(message, context)
								.equals("Restart")) {
							realRestart = runtime
									.findActiveRestart(restart);
						}
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("availableRestarts") {
					private final DefaultArgumentsDefinition ARGUMENTS = DefaultArgumentsDefinition
							.builder().withOptionalPositional("condition",
									"Condition")
							.getArguments();
					...
					public Object activate(IokeObject method,
						...
						for (List<Runtime.RestartInfo> lri : activeRestarts) {
							for (Runtime.RestartInfo rri : lri) {
								if (IokeObject.isTrue(Interpreter.send(
										runtime.callMessage, context,
										Interpreter.send(
												runtime.testMessage,
												context, rri.restart),
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("signal!") {
					private final DefaultArgumentsDefinition ARGUMENTS = DefaultArgumentsDefinition
							.builder().withRequiredPositional("datum")
					...
					public Object activate(IokeObject method,
						...
						return signal(datum, positionalArgs, keywordArgs,
								message, context);
					}
				}));
				...
				new NativeMethod("error!") {
					...
					public Object activate(IokeObject method,
						...
						if (IokeObject.data(datum) instanceof Text) {
							...
							datum = IokeObject.as(IokeObject
									.as(context.runtime.condition.getCell(
											message, context, "Error"),
											context)
									.getCell(message, context, "Default"),
									context).mimic(message, context);
							IokeObject.setCell(datum, message, context,
									"text", oldDatum);
						}

						IokeObject condition = signal(datum,
								positionalArgs, keywordArgs, message,
								context);
						IokeObject err = IokeObject
								.as(context.runtime.system.getCell(message,
										context, "err"), context);

						Interpreter.send(context.runtime.printMessage,
								context, err,
								context.runtime.newText("*** - "));
						Interpreter.send(context.runtime.printlnMessage,
								context, err,
								Interpreter.send(
										context.runtime.reportMessage,
										context, condition));

						IokeObject currentDebugger = IokeObject.as(
								Interpreter.send(
										context.runtime.currentDebuggerMessage,
										context, context.runtime.system),
								context);

						if (!currentDebugger.isNil()) {
							Interpreter.send(context.runtime.invokeMessage,
									context, currentDebugger, condition,
									context);
						}
