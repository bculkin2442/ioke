...
public class Range extends IokeData {
	...
	private static class RangeIterator implements Iterator<Object> {
		...
		public RangeIterator(IokeObject start, IokeObject end,
				boolean inclusive, boolean inverted, IokeObject context,
				IokeObject message) {
		...
		public boolean hasNext() {
			try {
				boolean sameEndpoints = IokeObject.isTrue(Interpreter
						.send(runtime.eqMessage, context, start, end));
				boolean shouldGoOver = (doLast && inclusive);
				boolean sameStartPoint = sameEndpoints && inclusive
						&& !oneIteration;
		...
		public Object next() {
			...
			try {
				if (!IokeObject.isTrue(Interpreter.send(runtime.eqMessage,
						context, start, end))) {
					oneIteration = true;
					start = (IokeObject) Interpreter.send(messageToSend,
							context, start);
					doLast = true;
					return obj;
				} else {
	...
	public void init(IokeObject obj) throws ControlFlow {
		...
		obj.registerMethod(runtime.newNativeMethod(
				"returns true if the left hand side range is equal to the right hand side range.",
				new TypeCheckingNativeMethod("==") {
					private final TypeCheckingArgumentsDefinition ARGUMENTS = TypeCheckingArgumentsDefinition
							.builder().receiverMustMimic(runtime.range)
					...
					public Object activate(IokeObject self, Object on,
						...
						return ((other instanceof IokeObject)
								&& (IokeObject
										.data(other) instanceof Range)
								&& d.inclusive == ((Range) IokeObject
										.data(other)).inclusive
								&& d.from.equals(((Range) IokeObject
										.data(other)).from)
								&& d.to.equals(((Range) IokeObject
										.data(other)).to))
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("inclusive") {
					private final DefaultArgumentsDefinition ARGUMENTS = DefaultArgumentsDefinition
							.builder().withRequiredPositional("from")
							.withRequiredPositional("to").getArguments();
					...
					public Object activate(IokeObject method,
						...
						boolean comparing = IokeObject.isMimic(from,
								IokeObject.as(context.runtime.mixins.body
										.get("Comparing"), context),
								context);
						boolean inverted = false;

						if (comparing) {
							Object result = Interpreter.send(
									context.runtime.spaceShip, context,
									from, to);
							if (result != context.runtime.nil
									&& Number.extractInt(result, message,
											context) == 1) {
								inverted = true;
							}
						}
						...
						return runtime.newRange(
								IokeObject.as(from, context),
								IokeObject.as(to, context), true,
								inverted);
					}
				}));
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("exclusive") {
					...
					public Object activate(IokeObject method,
						...
						return runtime.newRange(
								IokeObject.as(from, context),
								IokeObject.as(to, context), false,
								inverted);
					}
				}));
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new TypeCheckingNativeMethod.WithNoArguments("from",
						runtime.range) {
					@Override
		...
		obj.registerMethod(obj.runtime.newNativeMethod(
				"returns a new sequence to iterate over this range",
				new TypeCheckingNativeMethod.WithNoArguments("seq",
					...
					public Object activate(IokeObject method, Object on,
						...
						RangeIterator ri = new RangeIterator(r.from, r.to,
								r.inclusive, r.inverted, context, message);
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("each") {
					...
					public Object activate(IokeObject method,
						...
						IokeObject from = IokeObject.as(
								((Range) IokeObject.data(on)).from,
								context);
						IokeObject to = IokeObject.as(
								((Range) IokeObject.data(on)).to, context);
						boolean inclusive = ((Range) IokeObject
								.data(on)).inclusive;
						...
						switch (message.getArgumentCount()) {
							...
							case 1: {
								...
								while (!IokeObject.isTrue(Interpreter.send(
										context.runtime.eqMessage, context,
										current, to))) {
									runtime.interpreter.evaluate(code,
											context,
											context.getRealContext(),
											current);
									current = Interpreter.send(
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("===") {
					...
					public Object activate(IokeObject method,
						...
						IokeObject to = IokeObject.as(
								((Range) IokeObject.data(on)).to, context);
						boolean comparing = IokeObject
								.isMimic(from,
										IokeObject.as(
												context.runtime.mixins.body
														.get("Comparing"),
												context));
						boolean inclusive = ((Range) IokeObject
								.data(on)).inclusive;
						...
						if (comparing) {
							...
							if (IokeObject
									.isTrue(Interpreter.send(firstMessage,
											context, from, other))
									&& ((inclusive && IokeObject
											.isTrue(Interpreter.send(
													secondMessageInclusive,
													context, to, other)))
											|| IokeObject.isTrue(
													Interpreter.send(
															secondMessageExclusive,
															context, to,
															other)))) {
							...
							while (!IokeObject.isTrue(Interpreter.send(
								...
								current = Interpreter.send(messageToSend,
										context, current);
							}

							if (inclusive && IokeObject.isTrue(Interpreter
									.send(context.runtime.eqMessage,
											context, to, other))) {
								return context.runtime._true;
							}
							return context.runtime._false;
