...
public class TypeCheckingArgumentsDefinition
	...
	private TypeCheckingArgumentsDefinition(List<Argument> arguments,
			...
			List<TypeChecker> mustMimic, TypeChecker receiverMustMimic) {
	...
	public Object getValidatedArgumentsAndReceiver(IokeObject context,
		...
		for (int i = 0, j = this.arguments.size(); i < j; i++) {
			...
			if (a instanceof KeywordArgument) {
				...
				if (given != null) {
					givenKeywords.put(name,
							mustMimic.get(0).convertToMimic(given, message,
									context, true));
				}
			} else {
				if (ix < argumentsWithoutKeywords.size()) {
					argumentsWithoutKeywords.set(ix,
							mustMimic.get(i).convertToMimic(
		...
		return receiverMustMimic.convertToMimic(on, message, context,
				true);
	}

	public static TypeCheckingArgumentsDefinition empty() {
		return new TypeCheckingArgumentsDefinition(
				new ArrayList<Argument>(), new ArrayList<String>(), null,
				null, 0, 0, false, new ArrayList<TypeChecker>(),
				TypeChecker.None);
	}

	public static TypeCheckingArgumentsDefinition emptyButReceiverMustMimic(
			Object mimic) {
		return new TypeCheckingArgumentsDefinition(
				new ArrayList<Argument>(), new ArrayList<String>(), null,
				null, 0, 0, false, new ArrayList<TypeChecker>(),
	...
	public static class Builder
		...
		public Builder or(IokeObject mimic) {
			...
			mustMimic.set(ix,
					new TypeChecker.Or(mustMimic.get(ix), mimic));
			return this;
		}

		public Builder orBeNil() {
			int ix = mustMimic.size() - 1;
			mustMimic.set(ix, new TypeChecker.Or(mustMimic.get(ix),
					TypeChecker.Nil));
			return this;
		}
