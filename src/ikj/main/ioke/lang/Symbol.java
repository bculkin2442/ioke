...
public class Symbol extends IokeData {
		...
		obj.registerMethod(obj.runtime.newNativeMethod(
				...
				new TypeCheckingNativeMethod("==") {
					...
					public Object activate(IokeObject self, Object on,
						...
						return on == other ? context.runtime._true
				...
				new TypeCheckingNativeMethod("<=>") {
					...
					public Object activate(IokeObject method, Object on,
						...
						if (!(IokeObject.data(arg) instanceof Symbol)) {
							arg = IokeObject.convertToSymbol(arg, message,
									context, false);
							if (!(IokeObject
									.data(arg) instanceof Symbol)) {
								// Can't compare, so bail out
								return context.runtime.nil;
							}
						}
						return context.runtime.newNumber(Symbol.getText(on)
								.compareTo(Symbol.getText(arg)));
					}
				}));
	}
	...
	public void checkMimic(IokeObject obj, IokeObject m,
		...
		final IokeObject condition = IokeObject
				.as(IokeObject.getCellChain(context.runtime.condition, m,
						context, "Error", "CantMimicOddball"), context)
				.mimic(m, context);
	...
	public final static Pattern BAD_CHARS = Pattern
			.compile("[!=\\.:\\-\\+&|\\{\\[]");
