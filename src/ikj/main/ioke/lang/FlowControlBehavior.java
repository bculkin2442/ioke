...
public class FlowControlBehavior {
	public static void init(IokeObject obj) throws ControlFlow {
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("let") {
					...
					public Object activate(IokeObject method,
						...
						IokeObject lc = context.runtime.newLexicalContext(
						...
						try {
							while (ix < end) {
								IokeObject place = IokeObject
										.as(args.get(ix++), context);

								if (Message.next(place) == null && place
										.getArguments().size() == 0) {
									...
									while (Message
										...
										if (Message.next(Message.next(
												realPlace)) == null) {
											IokeObject temp = Message
													.next(realPlace);
											Message.setNext(realPlace,
													null);
											realPlace = temp;
										} else {
											realPlace = Message
									...
									Object wherePlace = context
											.getRealContext();
									if (place != realPlace) {
										wherePlace = Interpreter
									...
									if (realPlace.getArguments()
										...
										IokeObject msg = context.runtime
												.newMessageFrom(realPlace,
														newName,
														arguments);
										Interpreter.send(msg, context,
												wherePlace);
										valuesToUnbind.add(0,
												new Object[] { wherePlace,
														originalValue,
														realPlace });
									} else {
										Object value = Interpreter
												.getEvaluatedArgument(
														message, ix++,
														context);
										IokeObject.assign(wherePlace,
												realPlace.getName(), value,
												context, message);
										valuesToUnbind.add(0,
												new Object[] { wherePlace,
														originalValue,
							...
							while (!valuesToUnbind.isEmpty()) {
								try {
									Object[] vals = valuesToUnbind
											.remove(0);
									IokeObject wherePlace = IokeObject
											.as(vals[0], context);
									Object value = vals[1];
									IokeObject realPlace = IokeObject
											.as(vals[2], context);
				...
				new NativeMethod("break") {
					...
					public Object activate(IokeObject method,
						...
						if (message.getArgumentCount() > 0) {
							value = Interpreter.getEvaluatedArgument(
									message, 0, context);
						}
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("until") {
					private final DefaultArgumentsDefinition ARGUMENTS = DefaultArgumentsDefinition
							.builder()
							.withOptionalPositionalUnevaluated("condition")
							.withRestUnevaluated("body").getArguments();
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("loop") {
					private final DefaultArgumentsDefinition ARGUMENTS = DefaultArgumentsDefinition
							.builder().withRestUnevaluated("body")
							.getArguments();
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("if") {
					private final DefaultArgumentsDefinition ARGUMENTS = DefaultArgumentsDefinition
							.builder().withRequiredPositional("condition")
					...
					public Object activate(IokeObject method,
						...
						Object test = Interpreter
								.getEvaluatedArgument(message, 0, context);

						IokeObject itContext = context.runtime
								.newLexicalContext(
										context.getRealContext(),
										"Lexical activation context",
										context);
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("ensure") {
					...
					public Object activate(IokeObject method,
						...
						try {
							IokeObject msg = IokeObject.as(args.get(0),
									context);
							result = runtime.interpreter.evaluate(msg,
									context, context.getRealContext(),
									context);
						} finally {
							for (Object o : args.subList(1, argCount)) {
								IokeObject msg = IokeObject.as(o, context);
								runtime.interpreter.evaluate(msg, context,
