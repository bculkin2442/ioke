...
public final class IokeObject implements TypeChecker {
	...
	private void checkFrozen(String modification, IokeObject message,
		...
		if (isFrozen()) {
			final IokeObject condition = as(
					IokeObject.getCellChain(context.runtime.condition,
							message, context, "Error", "ModifyOnFrozen"),
					context).mimic(message, context);
			condition.setCell("message", message);
			condition.setCell("context", context);
			condition.setCell("receiver", this);
			condition.setCell("modification",
					context.runtime.getSymbol(modification));
			context.runtime.errorCondition(condition);
		}
	}

	public void become(IokeObject other, IokeObject message,
			IokeObject context) throws ControlFlow {
	...
	public void setDocumentation(String docs, IokeObject message,
			IokeObject context) throws ControlFlow {
		...
		switch (index) {
			...
			case 0:
				if (body.mimicCount-- == 2) {
					...
					System.arraycopy(body.mimics, 1, newMimics, 0,
							body.mimicCount);
			...
			default:
				if (index == body.mimicCount - 1) {
					...
					System.arraycopy(body.mimics, index + 1, newMimics,
							index, body.mimicCount - (index + 1));
					body.mimics = newMimics;
					body.mimicCount--;
				}
				break;
	...
	public static void removeMimic(Object on, Object other,
	...
	public static void removeAllMimics(Object on, IokeObject message,
			IokeObject context) throws ControlFlow {
		IokeObject me = as(on, context);
		...
		if (me.body.mimicCount == 1) {
			...
			while (me.body.mimicCount > 0) {
				Hook.fireMimicsChanged(me, message, context,
						me.body.mimics[me.body.mimicCount - 1]);
				Hook.fireMimicRemoved(me, message, context,
						me.body.mimics[me.body.mimicCount - 1]);
				me.body.mimicCount--;
			}
		}
	...
	public IokeObject allocateCopy(IokeObject m, IokeObject context) {
		return new IokeObject(runtime, null,
				data.cloneData(this, m, context));
	}

	public static Object findSuperCellOn(Object obj, IokeObject early,
			IokeObject context, String name) {
		if (name == null) {
			throw new RuntimeException(
					"can't be asked to find the super cell named null");
		}

		return as(obj, context).markingFindSuperCell(early, name,
				new boolean[] { false });
	}

	protected final Object realMarkingFindSuperCell(IokeObject early,
	...
	protected final Object markingFindSuperCell(IokeObject early,
		...
		if (nn == runtime.nul && isLexical()) {
			return ((LexicalContext) this.data).surroundingContext
					.realMarkingFindSuperCell(early, name,
							new boolean[] { false });
		}
		return nn;
	}
	...
	public static Object findPlace(Object obj, IokeObject m,
			IokeObject context, String name) throws ControlFlow {
		Object result = findPlace(obj, name);
		if (result == m.runtime.nul) {
			final IokeObject condition = as(
					IokeObject.getCellChain(m.runtime.condition, m,
							context, "Error", "NoSuchCell"),
	...
	/**
	 * Finds the first object in the chain where name is available as a
	 * cell, or nul if nothing can be found. findPlace is cycle aware and
	 * will not loop in an infinite chain. subclasses should copy this
	 * behavior.
	 */
	...
	public static final Object findCell(Object on, IokeObject context,
			String name) {
		return findCell(as(on, context), name);
	}

	public static IokeObject mimic(Object on, IokeObject message,
			IokeObject context) throws ControlFlow {
	...
	public static boolean isMimic(IokeObject on,
			IokeObject potentialMimic) {
	...
	public static Object getCellChain(Object on, IokeObject m,
			IokeObject c, String... names) throws ControlFlow {
		Object current = on;
	...
	public static Object getCell(Object on, IokeObject m,
			IokeObject context, String name) throws ControlFlow {
		return ((IokeObject) on).getCell(m, context, name);
	}

	public static Object setCell(Object on, IokeObject m,
			IokeObject context, String name, Object value) {
		((IokeObject) on).setCell(name, value);
		return value;
	}

	public static void removeCell(Object on, IokeObject m,
			IokeObject context, String name) throws ControlFlow {
		((IokeObject) on).removeCell(m, context, name);
	}

	public static void undefineCell(Object on, IokeObject m,
			IokeObject context, String name) throws ControlFlow {
	...
	public Object getCell(IokeObject m, IokeObject context, String name)
		...
		while (cell == runtime.nul) {
			final IokeObject condition = as(IokeObject.getCellChain(
					runtime.condition, m, context, "Error", "NoSuchCell"),
					context).mimic(m, context);
			condition.setCell("message", m);
			condition.setCell("context", context);
			condition.setCell("receiver", this);
	...
	public final static Pattern SLIGHTLY_BAD_CHARS = Pattern
			.compile("[!=\\.\\-\\+&|\\{\\[]");

	public final void assign(String name, Object value, IokeObject context,
		...
		if (isLexical()) {
			...
			if (!SLIGHTLY_BAD_CHARS.matcher(name).find()
					&& findCell(this, name + "=") != runtime.nul) {
				IokeObject msg = runtime.createMessage(new Message(runtime,
						name + "=", runtime.createMessage(
								Message.wrap(as(value, context)))));
				...
				if (body.hooks != null) {
					...
					Hook.fireCellChanged(this, message, context, name,
							prev);
				} else {
					body.put(name, value);
				}
	...
	public List<IokeObject> getMimics() {
		switch (body.mimicCount) {
			...
			default:
				return Arrays.asList(body.mimics).subList(0,
						body.mimicCount);
		}
	}
		...
		switch (body.mimicCount) {
			...
			default:
				if (at == 0) {
					...
					System.arraycopy(body.mimics, 0, newMimics, 1,
							newLen - 1);
					body.mimics = newMimics;
					newMimics[0] = mimic;
					body.mimicCount++;
					...
					if (body.mimicCount == body.mimics.length) {
						IokeObject[] newMimics = new IokeObject[body.mimics.length
								+ 1];
						System.arraycopy(body.mimics, 0, newMimics, 0,
								body.mimics.length);
						body.mimics = newMimics;
					}
					...
					if (body.mimicCount == body.mimics.length) {
						...
						System.arraycopy(body.mimics, at, newMimics,
								at + 1, body.mimicCount - at);
						body.mimics = newMimics;
						body.mimics[at] = mimic;
					} else {
						System.arraycopy(body.mimics, at, body.mimics,
								at + 1, body.mimicCount - at);
						body.mimics[at] = mimic;
					}
					body.mimicCount++;
	...
	public void singleMimics(IokeObject mimic, IokeObject message,
			IokeObject context) throws ControlFlow {
		checkFrozen("mimic!", message, context);
	...
	public void mimics(IokeObject mimic, IokeObject message,
			IokeObject context) throws ControlFlow {
		checkFrozen("mimic!", message, context);
	...
	public void mimics(int index, IokeObject mimic, IokeObject message,
			IokeObject context) throws ControlFlow {
	...
	public static Object convertTo(String kind, Object on,
			boolean signalCondition, String conversionMethod,
			IokeObject message, IokeObject context) throws ControlFlow {
		return ((IokeObject) on).convertTo(kind, signalCondition,
				conversionMethod, message, context);
	}

	public static Object convertTo(Object mimic, Object on,
			boolean signalCondition, String conversionMethod,
			IokeObject message, IokeObject context) throws ControlFlow {
		return ((IokeObject) on).convertTo(mimic, signalCondition,
				conversionMethod, message, context);
	}

	public static IokeObject convertToRational(Object on, IokeObject m,
			IokeObject context, boolean signalCondition)
			throws ControlFlow {
		return ((IokeObject) on).convertToRational(m, context,
				signalCondition);
	}

	public static IokeObject convertToDecimal(Object on, IokeObject m,
			IokeObject context, boolean signalCondition)
			throws ControlFlow {
		return ((IokeObject) on).convertToDecimal(m, context,
				signalCondition);
	}

	public Object convertTo(String kind, boolean signalCondition,
			String conversionMethod, IokeObject message,
			IokeObject context) throws ControlFlow {
		Object result = data.convertTo(this, kind, false, conversionMethod,
				message, context);
		if (result == null) {
			if (conversionMethod != null && findCell(this,
					conversionMethod) != context.runtime.nul) {
				IokeObject msg = context.runtime
						.newMessage(conversionMethod);
				return Interpreter.send(msg, context, this);
			}
			if (signalCondition) {
				return data.convertTo(this, kind, true, conversionMethod,
						message, context);
			}
	...
	public Object convertTo(Object mimic, boolean signalCondition,
			String conversionMethod, IokeObject message,
			IokeObject context) throws ControlFlow {
		Object result = data.convertTo(this, mimic, false,
				conversionMethod, message, context);
		if (result == null) {
			if (conversionMethod != null && findCell(this,
					conversionMethod) != context.runtime.nul) {
			...
			if (signalCondition) {
				return data.convertTo(this, mimic, true, conversionMethod,
						message, context);
			}
			return context.runtime.nul;
		}
	...
	public Object convertToThis(Object on, IokeObject message,
			IokeObject context) throws ControlFlow {
		return convertToThis(on, true, message, context);
	}

	public Object convertToThis(Object on, boolean signalCondition,
			IokeObject message, IokeObject context) throws ControlFlow {
		if (on instanceof IokeObject) {
	...
	public static Object ensureTypeIs(Class<?> clazz, IokeObject self,
			Object on, final IokeObject context, IokeObject message)
			throws ControlFlow {
		final Object[] receiver = new Object[] { on };
		while (!clazz.isInstance(IokeObject.data(receiver[0]))) {
			final IokeObject condition = as(IokeObject.getCellChain(
					context.runtime.condition, message, context, "Error",
					"Type", "IncorrectType"), context).mimic(message,
							context);
			condition.setCell("message", message);
			condition.setCell("context", context);
	...
	public IokeObject convertToRational(IokeObject m, IokeObject context,
			boolean signalCondition) throws ControlFlow {
		IokeObject result = data.convertToRational(this, m, context,
				false);
		if (result == null) {
			if (findCell(this, "asRational") != context.runtime.nul) {
				return IokeObject.as(Interpreter
						.send(context.runtime.asRational, context, this),
						context);
			}
			if (signalCondition) {
	...
	public IokeObject convertToDecimal(IokeObject m, IokeObject context,
			boolean signalCondition) throws ControlFlow {
		IokeObject result = data.convertToDecimal(this, m, context, false);
		if (result == null) {
			if (findCell(this, "asDecimal") != context.runtime.nul) {
				return IokeObject.as(Interpreter
						.send(context.runtime.asDecimal, context, this),
						context);
			}
			if (signalCondition) {
	...
	public static IokeObject convertToText(Object on, IokeObject m,
			IokeObject context, boolean signalCondition)
			throws ControlFlow {
	...
	public static IokeObject tryConvertToText(Object on, IokeObject m,
			IokeObject context) throws ControlFlow {
		return ((IokeObject) on).tryConvertToText(m, context);
	}

	public static IokeObject convertToSymbol(Object on, IokeObject m,
			IokeObject context, boolean signalCondition)
			throws ControlFlow {
	...
	public static IokeObject convertToRegexp(Object on, IokeObject m,
			IokeObject context) throws ControlFlow {
	...
	public IokeObject convertToText(IokeObject m, IokeObject context,
		...
		if (result == null) {
			if (findCell(this, "asText") != context.runtime.nul) {
				return as(Interpreter.send(context.runtime.asText, context,
						this), context);
			}
			if (signalCondition) {
	...
	public IokeObject convertToSymbol(IokeObject m, IokeObject context,
			boolean signalCondition) throws ControlFlow {
		IokeObject result = data.convertToSymbol(this, m, context, false);
		if (result == null) {
			if (findCell(this, "asSymbol") != context.runtime.nul) {
				return as(Interpreter.send(context.runtime.asSymbol,
