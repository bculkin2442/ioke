...
public abstract class NativeMethod extends Method implements CanRun {
	public static class WithNoArguments extends NativeMethod {
		private final static DefaultArgumentsDefinition ARGUMENTS = DefaultArgumentsDefinition
	...
	public void init(IokeObject javaMethod) throws ControlFlow {
		...
		javaMethod.registerMethod(javaMethod.runtime.newNativeMethod(
				...
				new WithNoArguments("argumentsCode") {
					@Override
					public Object activate(IokeObject self,
						...
						if (data instanceof NativeMethod) {
							return dynamicContext.runtime
									.newText(((NativeMethod) data)
											.getArgumentsCode());
						} else {
							return dynamicContext.runtime
									.newText(((AssociatedCode) data)
											.getArgumentsCode());
						}
					}
				}));
	...
	public String inspect(Object self) {
		return "method(" + getDominantClassName()
				+ ((name != null) ? ("_" + name) : "") + ")";
	}
}
