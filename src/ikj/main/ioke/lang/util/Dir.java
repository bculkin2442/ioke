...
public class Dir {
	public final static boolean	DOSISH				= System
	...
	public final static int		FNM_SYSCASE			= CASEFOLD_FILESYSTEM
			? FNM_CASEFOLD : 0;
	...
	private static int fnmatch_helper(String bytes, int pstart, int pend,
		...
		while (pat < pend) {
			...
			switch (c) {
				case '?':
					if (s >= send
							|| (pathname && isdirsep(string.charAt(s)))
							|| (period && string.charAt(s) == '.'
									&& (s == 0 || (pathname && isdirsep(
											string.charAt(s - 1)))))) {
						return FNM_NOMATCH;
					}
					s++;
					...
					while (s < send) {
						if ((c == '?' || c == '['
								|| Character.toLowerCase(
										string.charAt(s)) == test)
								&& fnmatch(bytes, pat, pend, string, s,
										send, flags | FNM_DOTMATCH) == 0) {
							return 0;
						} else if ((pathname
				...
				case '[':
					...
					break;
				case '\\':
					if (escape && (!DOSISH || (pat < pend && "*?[]\\"
							.indexOf(bytes.charAt(pat)) != -1))) {
						if (pat >= pend) {
							c = '\\';
						} else {
							c = bytes.charAt(pat++);
	...
	public static int fnmatch(String bytes, int pstart, int pend,
			String string, int sstart, int send, int flags) {
		...
		if (pathname) {
			while (true) {
				...
				if (fnmatch_helper(bytes, pat_pos, patSlashIdx, string,
						str_pos, strSlashIdx, flags) == 0) {
	...
	private static boolean isDoubleStarAndSlash(String bytes, int pos) {
		...
		return bytes.charAt(pos) == '*' && bytes.charAt(pos + 1) == '*'
				&& bytes.charAt(pos + 2) == '/';
	}
	...
	private static int nextSlashIndex(String bytes, int start, int end) {
		int idx = start;
		while (idx < end && idx < bytes.length()
				&& bytes.charAt(idx) != '/') {
			idx++;
		}
		return idx;
	...
	public static int range(String _pat, int pat, int pend, char test,
			int flags) {
		boolean not;
		boolean ok = false;
		boolean nocase = (flags & FNM_CASEFOLD) != 0;
	...
	private static int push_braces(String cwd, List<String> result,
			GlobPattern pattern) {
		pattern.reset();
		int lbrace = pattern.indexOf('{'); // index of left-most brace
		int rbrace = pattern.findClosingIndexOf(lbrace);// index of
														// right-most brace
	...
	private static int push_globs(String cwd, List<String> ary,
			GlobPattern pattern) {
		pattern.flags |= FNM_SYSCASE;
		return glob_helper(cwd, pattern.string, pattern.begin, pattern.end,
				-1, pattern.flags, glob_caller,
				new GlobArgs(push_pattern, ary));
	}

	private static boolean has_magic(String bytes, int begin, int end,
			int flags) {
		boolean escape = (flags & FNM_NOESCAPE) == 0;
		boolean nocase = (flags & FNM_CASEFOLD) != 0;
		int open = 0;
	...
	private static int remove_backslashes(StringBuilder bytes, int index,
			int len) {
		int t = index;
	...
	private static boolean BASE(String base) {
		return DOSISH ? (base.length() > 0
				&& !((isdirsep(base.charAt(0)) && base.length() < 2)
						|| (base.length() > 2 && base.charAt(1) == ':'
								&& isdirsep(base.charAt(2))
								&& base.length() < 4)))
				: (base.length() > 0 && !(isdirsep(base.charAt(0))
						&& base.length() < 2));
	}

	private static boolean isJarFilePath(String bytes, int begin,
			int end) {
		return end > 6
				&& bytes.substring(begin, begin + 5).equals("file:");
	}
	...
	private static int glob_helper(String cwd, String _bytes, int begin,
		...
		if (!has_magic(_bytes, p, end, flags)) {
			...
			if (bytes.charAt(begin) == '/' || (DOSISH && begin + 2 < end
					&& bytes.charAt(begin + 1) == ':'
					&& isdirsep(bytes.charAt(begin + 2)))) {
				if (new File(bytes.subSequence(begin, end).toString())
						.exists()) {
				...
				String jar = bytes.subSequence(begin + ix + 1, end)
						.toString();
				try {
					...
					// We don't care
											...
											// a filename ever.
				if (new File(cwd, bytes.subSequence(begin, end).toString())
						.exists()) {
					status = func.call(bytes.toString(), begin,
							end - begin, arg);
				}
			}
		...
		mainLoop: while (p != -1 && status == 0) {
			...
			if (has_magic(bytes2, p, m == -1 ? end : m, flags)) {
				finalize: do {
					...
					if (dir.charAt(0) == '/' || (DOSISH && 2 < dir.length()
							&& dir.charAt(1) == ':'
							&& isdirsep(dir.charAt(2)))) {
						st = new File(dir);
					...
					if ((jf != null && ("".equals(jar)
							|| (jf.getJarEntry(jar) != null
									&& jf.getJarEntry(jar).isDirectory())))
							|| st.isDirectory()) {
						if (m != -1 && magic.equals(DOUBLE_STAR)) {
							int n = base.length();
							recursive = true;
							buf = base + bytes2.substring(
									(base.length() > 0 ? m : m + 1), end);
							status = glob_helper(cwd, buf, 0, buf.length(),
									n, flags, func, arg);
							if (status != 0) {
								break finalize;
							}
					...
					if (jar == null) {
						...
						for (int i = 0; i < dirp.length; i++) {
							if (recursive) {
								String bs = dirp[i];
								if (fnmatch(STAR, 0, 1, bs, 0, bs.length(),
										flags) != 0) {
									continue;
								}
								buf = base + (BASE(base) ? SLASH : EMPTY);
								buf += dirp[i];
								if (buf.charAt(0) == '/' || (DOSISH
										&& 2 < buf.length()
										&& buf.charAt(1) == ':'
										&& isdirsep(buf.charAt(2)))) {
									st = new File(buf);
								} else {
									st = new File(cwd, buf);
								}
								if (st.isDirectory()
									...
									status = glob_helper(cwd, buf, 0,
											buf.length(), t, flags, func,
											arg);
									if (status != 0) {
										break;
									}
						...
						try {
							...
							for (JarEntry je : dirp) {
								...
								if (recursive) {
									if (fnmatch(STAR, 0, 1, bs, 0, len,
											flags) != 0) {
										continue;
									}
									buf = base.substring(0,
											base.length() - jar.length());
								...
								if (fnmatch(magic, 0, magic.length(), bs,
										0, len, flags) == 0) {
									buf = base.substring(0,
											base.length() - jar.length());
									buf += (BASE(base) ? SLASH : EMPTY);
				...
				if (link.size() > 0) {
					for (String b : link) {
						if (status == 0) {
							if (b.charAt(0) == '/' || (DOSISH
									&& 2 < b.length() && b.charAt(1) == ':'
									&& isdirsep(b.charAt(2)))) {
								st = new File(b);
							} else {
								st = new File(cwd, b);
