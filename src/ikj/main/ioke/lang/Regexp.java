...
public class Regexp extends IokeData {
	...
	public void init(IokeObject obj) throws ControlFlow {
		...
		final IokeObject regexpMatch = new IokeObject(runtime,
				"contains behavior related to assignment",
				new RegexpMatch(obj, null, null));
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new TypeCheckingNativeMethod("==") {
					private final TypeCheckingArgumentsDefinition ARGUMENTS = TypeCheckingArgumentsDefinition
							.builder().receiverMustMimic(runtime.regexp)
					...
					public Object activate(IokeObject self, Object on,
						...
						return (((other instanceof IokeObject)
								&& (IokeObject
										.data(other) instanceof Regexp)
								&& ((on == context.runtime.regexp
										|| other == context.runtime.regexp)
												? on == other
												: (((Regexp) IokeObject
														.data(on)).pattern
																.equals(((Regexp) IokeObject
																		.data(other)).pattern)
														&& ((Regexp) IokeObject
																.data(on)).flags
																		.equals(((Regexp) IokeObject
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new TypeCheckingNativeMethod.WithNoArguments("pattern",
						runtime.regexp) {
					@Override
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new TypeCheckingNativeMethod("match") {
					private final TypeCheckingArgumentsDefinition ARGUMENTS = TypeCheckingArgumentsDefinition
							.builder().receiverMustMimic(runtime.regexp)
							.withRequiredPositional("other")
							.getArguments();
					...
					public Object activate(IokeObject method, Object on,
							...
							IokeObject context, IokeObject message)
							throws ControlFlow {
						IokeObject target = IokeObject.as(
						...
						if (m.find()) {
							IokeObject match = regexpMatch
									.allocateCopy(message, context);
							match.singleMimicsWithoutCheck(regexpMatch);
							match.setData(new RegexpMatch(
									IokeObject.as(on, context), m,
									target));
							return match;
						} else {
				...
				new NativeMethod("quote") {
					...
					public Object activate(IokeObject method, Object on,
						...
						return context.runtime.newText(
								Pattern.quote(Text.getText(Interpreter
										.send(context.runtime.asText,
												context, args.get(0)))));
					}
				}));
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new NativeMethod("from") {
					private final DefaultArgumentsDefinition ARGUMENTS = DefaultArgumentsDefinition
							.builder().withRequiredPositional("pattern")
					...
					public Object activate(IokeObject method, Object on,
						...
						String flags = "";
						if (args.size() > 1) {
							flags = Text.getText(Interpreter.send(
									context.runtime.asText, context,
									args.get(1)));
						}

						return context.runtime.newRegexp(pattern, flags,
								context, message);
					}
				}));
		...
		obj.registerMethod(runtime.newNativeMethod(
				...
				new TypeCheckingNativeMethod("allMatches") {
					private final TypeCheckingArgumentsDefinition ARGUMENTS = TypeCheckingArgumentsDefinition
							.builder().receiverMustMimic(runtime.regexp)
							.withRequiredPositional("other")
							.getArguments();
					...
					public Object activate(IokeObject method, Object on,
						...
						while (iter.hasMore()) {
							result.add(runtime
									.newText(iter.nextMatch().group(0)));
						}
