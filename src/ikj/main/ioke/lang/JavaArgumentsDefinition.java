...
public class JavaArgumentsDefinition {
	...
	public JavaArgumentsDefinition(Member[] members,
			Class[][] parameterTypes, int min, int max) {
		this(members, parameterTypes, min, max, false);
	}

	public JavaArgumentsDefinition(Member[] members,
			Class[][] parameterTypes, int min, int max, boolean special) {
		// System.err.println("creating a new arguments definition: " +
		// Arrays.asList((Object[])members) + "(" +
		// Arrays.asList((Object[])parameterTypes[0]) + ") min=" + min + ",
		// max=" + max);
	...
	private static class FullOrdering {
		protected int ordering(Class a, Class b) {
			// There are three choices. The first is when one argument is
			// strictly more specific than the other
			// The second is when the second argument is more specific than
			// the first
			...
			if (a == b) {
				...
				if (a == Integer.TYPE) {
					if (b == Short.TYPE || b == Character.TYPE
							|| b == Long.TYPE || b == Byte.TYPE) {
						return -1;
					}
				} else if (a == Short.TYPE) {
					if (b == Long.TYPE || b == Integer.TYPE
							|| b == Character.TYPE) {
						return 1;
					} else if (b == Byte.TYPE) {
						return -1;
					...
					if (b == Short.TYPE || b == Long.TYPE
							|| b == Integer.TYPE || b == Character.TYPE) {
						return 1;
					}
	...
	private static class MethodComparator extends FullOrdering
		...
		public int compare(Method a, Method b) {
			...
			// Comparison of each parameter in turn. The first one to be
			// different will decide the full ordering
	...
	public static JavaArgumentsDefinition createFrom(Field f) {
		...
		return new JavaArgumentsDefinition(new Field[] { f }, params, 1,
				1);
	}
	...
	private static class JavaArgumentDefinition {
		...
		public JavaArgumentDefinition(Class type, Class altType,
				Object obj) {
			this(type, altType, obj, false);
		}

		public JavaArgumentDefinition(Class type, Class altType,
				Object obj, boolean associatedCode) {
			this.type = type;
	...
	public Member getJavaArguments(IokeObject context, IokeObject message,
		...
		for (Object o : arguments) {
			if (Message.isKeyword(o)) {
				// ignore for now
			} else if (Message.hasName(o, "*") && IokeObject.as(o, context)
					.getArguments().size() == 1) { // Splat
				Object result = Interpreter.getEvaluatedArgument(
						IokeObject.as(o, context).getArguments().get(0),
						context);
				if (IokeObject.data(result) instanceof IokeList) {
					List<Object> elements = IokeList.getList(result);
					for (Object oe : elements) {
						resultArguments.add(new JavaArgumentDefinition(
								null, null, oe));
					}
					...
					List<Object> outp = IokeList
							.getList(runtime.withRestartReturningArguments(
									...
									new Restart.DefaultValuesGivingRestart(
											"ignoreArgument", runtime.nil,
											0),
									new Restart.DefaultValuesGivingRestart(
											"takeArgumentAsIs",
											IokeObject.as(result, context),
											1)));
				...
				Object result = Interpreter
						.getEvaluatedArgument(Message.next(o), context);
				Class into = null;
				Class alt = null;
				...
				resultArguments.add(
						new JavaArgumentDefinition(into, alt, result));
				argCount++;
			} else {
				Object val = Interpreter.getEvaluatedArgument(o, context);
				JavaArgumentDefinition jarg = new JavaArgumentDefinition(
						null, null, val,
						val instanceof IokeObject && IokeObject
								.data(val) instanceof AssociatedCode);
		...
		nextMethod: for (int j = parameterTypes.length; i < j; i++) {
			...
			if (parameterTypes[i].length == argCount) {
				...
				for (int kpar = (special ? 1
						: 0), karg = 0; kpar < argCount; kpar++, karg++) {
					Class clz = current[kpar];
					JavaArgumentDefinition jad = resultArguments.get(karg);
					Object obj = jad.obj;
					boolean isIokeObject = obj instanceof IokeObject;
					boolean isWrapper = isIokeObject
							&& IokeObject.data(obj) instanceof JavaWrapper;
					boolean isExplicitCast = jad.type != null;
					boolean clzIsAbstract = Modifier.isAbstract(
							clz.getModifiers()) || clz.isInterface();
					if (isExplicitCast
							&& !(clz == jad.type || clz == jad.altType)) {
						args.clear();
						continue nextMethod;
					}
					if (clz == String.class) {
						if (obj instanceof String) {
							args.add(obj);
						} else if (isWrapper && JavaWrapper
								.getObject(obj) instanceof String) {
							args.add(JavaWrapper.getObject(obj));
						} else if (isIokeObject
								&& IokeObject.data(obj) instanceof Text) {
							args.add(Text.getText(obj));
						} else if (isIokeObject && IokeObject
								.data(obj) instanceof Symbol) {
						...
						if (obj instanceof Character) {
							args.add(obj);
								...
								.getObject(obj) instanceof Character) {
							args.add(JavaWrapper.getObject(obj));
						} else if (isIokeObject && IokeObject
								.data(obj) instanceof Number) {
							args.add(new Character((char) Number
									.intValue(obj).intValue()));
						} else if (isIokeObject
								&& IokeObject.data(obj) instanceof Text
								&& Text.getText(obj).length() == 1) {
							args.add(new Character(
									Text.getText(obj).charAt(0)));
							...
							|| clz == Integer.TYPE) {
						// This should take into account widening and stuff
						// like that later
						if (obj instanceof Integer) {
							args.add(obj);
						} else if (isWrapper && JavaWrapper
								.getObject(obj) instanceof Integer) {
							args.add(JavaWrapper.getObject(obj));
						} else if (isIokeObject && IokeObject
								.data(obj) instanceof Number) {
							args.add(Integer.valueOf(
									Number.intValue(obj).intValue()));
						} else if (!clz.isPrimitive()
								&& obj == runtime.nil) {
							args.add(null);
						} else {
						...
						if (obj instanceof Short) {
							args.add(obj);
								...
								.getObject(obj) instanceof Short) {
							args.add(JavaWrapper.getObject(obj));
						} else if (isIokeObject && IokeObject
								.data(obj) instanceof Number) {
							args.add(Short.valueOf((short) Number
									.intValue(obj).intValue()));
						} else if (!clz.isPrimitive()
								&& obj == runtime.nil) {
							args.add(null);
						...
						if (obj instanceof Byte) {
							args.add(obj);
								...
								.getObject(obj) instanceof Byte) {
							args.add(JavaWrapper.getObject(obj));
						} else if (isIokeObject && IokeObject
								.data(obj) instanceof Number) {
							args.add(Byte.valueOf((byte) Number
									.intValue(obj).intValue()));
						} else if (!clz.isPrimitive()
								&& obj == runtime.nil) {
							args.add(null);
						...
						if (obj instanceof Long) {
							args.add(obj);
								...
								.getObject(obj) instanceof Long) {
							args.add(JavaWrapper.getObject(obj));
						} else if (isIokeObject && IokeObject
								.data(obj) instanceof Number) {
							args.add(Long.valueOf(
									Number.value(obj).longValue()));
						} else if (!clz.isPrimitive()
								&& obj == runtime.nil) {
							args.add(null);
						} else {
						...
						if (obj instanceof Float) {
							args.add(obj);
								...
								.getObject(obj) instanceof Float) {
							args.add(JavaWrapper.getObject(obj));
						} else if (isIokeObject && IokeObject
								.data(obj) instanceof Decimal) {
							args.add(Float.valueOf(
									Decimal.value(obj).floatValue()));
						} else if (isExplicitCast && IokeObject
								.data(obj) instanceof Number) {
							args.add(Float.valueOf(
									Number.value(obj).intValue()));
						} else if (!clz.isPrimitive()
								&& obj == runtime.nil) {
							args.add(null);
						} else {
						...
						if (obj instanceof Double) {
							args.add(obj);
								...
								.getObject(obj) instanceof Double) {
							args.add(JavaWrapper.getObject(obj));
						} else if (isIokeObject && IokeObject
								.data(obj) instanceof Decimal) {
							args.add(Double.valueOf(
									Decimal.value(obj).doubleValue()));
						} else if (isExplicitCast && IokeObject
								.data(obj) instanceof Number) {
							args.add(Double.valueOf(
									Number.value(obj).longValue()));
						...
						if (obj instanceof Boolean) {
							args.add(obj);
								...
								.getObject(obj) instanceof Boolean) {
							args.add(JavaWrapper.getObject(obj));
						...
						// System.err.println(" -- adding the object for: "
						// + members[i]);
						// System.err.println(" :" + clz);
						// System.err.println(" " + clz +
						// ".isAssignableFrom(" + obj.getClass() + "): " +
						// clz.isAssignableFrom(obj.getClass()));
						// System.err.println(" " + obj.getClass() +
						// ".isAssignableFrom(" + clz + "): " +
						// obj.getClass().isAssignableFrom(clz));
						...
						if (obj == runtime.nil) {
							args.add(null);
								...
								&& (clz.isAssignableFrom(obj.getClass())
										|| isExplicitCast)) {
							args.add(obj);
						} else if (isWrapper && (clz.isAssignableFrom(
								JavaWrapper.getObject(obj).getClass())
								|| isExplicitCast)) {
							args.add(JavaWrapper.getObject(obj));
						} else {
							args.clear();
						...
						// here should probably be more advanced matching
						// later on
						if (obj == runtime.nil) {
							args.add(null);
						} else if (!isIokeObject) {
							...
							try {
								Object obj2 = Interpreter.send(
										runtime.coerceIntoJavaCodeMessage,
										context, obj,
										runtime.registry.wrap(clz),
										findAbstractMethodNames(clz,
												context));
								if (obj2 instanceof IokeObject
										&& IokeObject.data(
												obj2) instanceof JavaWrapper) {
		...
		if (i == members.length) {
			final IokeObject condition = IokeObject
					.as(IokeObject.getCellChain(runtime.condition, message,
							context, "Error", "Java", "NoMatch"), context)
