...
public class Coverage {
	public static void processCoverage(ioke.lang.Runtime runtime,
			CoverageInterpreter citer) throws Throwable {
		...
		if (configFile.exists()) {
			...
			if (dir != runtime.nul && dir != runtime.nil) {
				directory = Text.getText(Interpreter.send(runtime.asText,
						runtime.ground, dir));
			}
			IokeObject fileFilter = (IokeObject) IokeObject
					.findCell(config, "files");
			if (fileFilter != runtime.nul && fileFilter != runtime.nil) {
				for (Iterator<String> iter = files.iterator(); iter
					...
					if (!IokeObject.isTrue(Interpreter.send(
							runtime.callMessage, runtime.ground,
							fileFilter, runtime.newText(iter.next())))) {
						iter.remove();
					}
				}
			}
		...
		PrintWriter summaryX = new PrintWriter(
				new FileWriter(new File(dir, "frame-summary.html")));
		StringWriter summaryS = new StringWriter();
		PrintWriter summary = new PrintWriter(summaryS);
		PrintWriter filesh = new PrintWriter(
				new FileWriter(new File(dir, "frame-files.html")));
		...
		for (String filename : files) {
			...
			if (new File(realFile).exists()) {
				...
				CoverageParser unparser = new CoverageParser(runtime,
						...
						runtime.ground, runtime.message, filename,
						cdata.get(filename), fw);
				...
				filesh.println("        <tr>");
				filesh.println(
						"          <td nowrap=\"nowrap\"><a target=\"summary\" href=\""
								+ filenameInFilesystem + ".html\">"
								+ filename2 + "</a> <i>("
								+ (unparser.numberOfCovered * 100
										/ unparser.messages)
								+ "%)</i></td>");
				filesh.println("        </tr>");
				...
				summary.println("        <td>");
				summary.println(
						"          <a href=\"" + filenameInFilesystem
								+ ".html\" onclick='parent.sourceFileList.location.href=\""
								+ filenameInFilesystem + ".html\"'>"
								+ filename2 + "</a>");
				summary.println("        </td>");
				summary.println("        <td>");
				...
				summary.println(
						"              <td align=\"right\" class=\"percentgraph\" width=\"40\">"
								+ unparser.percentageComplete + "%</td>");
				summary.println(
				...
				summary.println(
						"                  <div class=\"greenbar\" style=\"width:"
								+ unparser.percentageComplete + "px\">");
				summary.println("                    <span class=\"text\">"
						+ unparser.ratioComplete + "</span>");
				summary.println("                  </div>");
				summary.println("                </div>");
				summary.println("              </td>");
				summary.println("            </tr>");
				...
				summary.println(
						"                  <div class=\"greenbar\" style=\"width:"
								+ unparser.percentagePartial + "px\">");
				summary.println("                    <span class=\"text\">"
						+ unparser.ratioPartial + "</span>");
				summary.println("                  </div>");
				summary.println("                </div>");
				summary.println("              </td>");
				summary.println("            </tr>");
				...
				summary.println(
						"              <td align=\"right\" class=\"percentgraph\" width=\"40\">"
								+ unparser.percentageMessages + "%</td>");
				summary.println(
						"              <td class=\"percentgraph\">");
				summary.println(
						"                <div class=\"percentgraph\">");
				summary.println(
						"                  <div class=\"greenbar\" style=\"width:"
								+ unparser.percentageMessages + "px\">");
				summary.println("                    <span class=\"text\">"
						+ unparser.ratioMessages + "</span>");
				summary.println("                  </div>");
				summary.println("                </div>");
				summary.println("              </td>");
				summary.println("            </tr>");
		...
		int percentageComplete = linesCompletelyCovered * 100
				/ linesWithContent;
		int percentagePartial = linesPartiallyCovered * 100
				/ linesWithContent;
		int percentageMessages = numberOfCovered * 100 / messages;

		IokeObject ratioComplete = runtime
				.newNumber(RatNum.make(IntNum.make(linesCompletelyCovered),
						IntNum.make(linesWithContent)));
		IokeObject ratioPartial = runtime
				.newNumber(RatNum.make(IntNum.make(linesPartiallyCovered),
						IntNum.make(linesWithContent)));
		IokeObject ratioMessages = runtime.newNumber(RatNum.make(
		...
		summaryX.println("                    <span class=\"text\">"
				+ ratioComplete + "</span>");
		...
		int read = 0;
		FileOutputStream index = new FileOutputStream(
				new File(dir, "index.html"));
		InputStream fileis = Main.class
				.getResourceAsStream("/ioke/lang/coverage/index.html");
		...
		FileOutputStream css = new FileOutputStream(
				new File(dir, "main.css"));
		InputStream maincss = Main.class
				.getResourceAsStream("/ioke/lang/coverage/main.css");
