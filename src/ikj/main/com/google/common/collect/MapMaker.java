 ...
 * {@link Object#equals(Object) equals} method) to determine equality for
 * keys or values. However, if {@link #weakKeys()} or {@link #softKeys()}
 * was specified, the map uses identity ({@code ==}) comparisons instead
 * for keys. Likewise, if {@link #weakValues()} or {@link #softValues()}
 * was specified, the map uses identity comparisons for values.
 *
 * <p>
 * The returned map has <i>weakly consistent iteration</i>: an iterator
 ...
 * never observe a partially-reclaimed entry. Any
 * {@link java.util.Map.Entry} instance retrieved from the map's
 * {@linkplain Map#entrySet() entry set} is snapshot of that entry's state
 * at the time of retrieval.
 *
 * <p>
 * {@code new MapMaker().weakKeys().makeMap()} can almost always be used as
 * a drop-in replacement for {@link java.util.WeakHashMap}, adding
...
public final class MapMaker {
	 ...
	 *             if an initial capacity was already set (TODO: make that
	 *             true)
	 */
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             if {@code loadFactor} is nonpositive
	 * @throws IllegalStateException
	 *             if a load factor was already set (TODO: make that true)
	 */
	 ...
	 * @throws IllegalStateException
	 *             if a concurrency level was already set (TODO: make that
	 *             true)
	 */
	 ...
	 * determine equality of weak keys, which may not behave as you expect.
	 * For example, storing a key in the map and then attempting a lookup
	 * using a different but {@link Object#equals(Object)
	 * equals}-equivalent key will always fail.
	 *
	 * @throws IllegalStateException
	 ...
	 * <p>
	 * <b>Note:</b> the map will use identity ({@code ==}) comparison to
	 * determine equality of soft keys, which may not behave as you expect.
	 * For example, storing a key in the map and then attempting a lookup
	 * using a different but {@link Object#equals(Object)
	 * equals}-equivalent key will always fail.
	 *
	 ...
	 * <p>
	 * <b>Note:</b> the map will use identity ({@code ==}) comparison to
	 * determine equality of weak values. This will notably impact the
	 * behavior of {@link Map#containsValue(Object) containsValue},
	 * {@link ConcurrentMap#remove(Object, Object) remove(Object, Object)},
	 * and {@link ConcurrentMap#replace(Object, Object, Object) replace(K,
	 * V, V)}.
	 *
	 * @throws IllegalStateException
	 *             if the key strength was already set
	 ...
	 *
	 * <p>
	 * Soft values will be garbage collected in response to memory demand,
	 * and in a least-recently-used manner. This makes them a good
	 * candidate for caching.
	 *
	 * <p>
	 * <b>Note:</b> the map will use identity ({@code ==}) comparison to
	 * determine equality of soft values. This will notably impact the
	 * behavior of {@link Map#containsValue(Object) containsValue},
	 * {@link ConcurrentMap#remove(Object, Object) remove(Object, Object)},
	 * and {@link ConcurrentMap#replace(Object, Object, Object) replace(K,
	 * V, V)}.
	 ...
	 * {@link Map#get} returns the value corresponding to the given key,
	 * atomically computes it using the computer function passed to this
	 * builder, or waits for another thread to compute the value if
	 * necessary. Only one value will be computed for each key at a given
	 * time.
	 *
	 * <p>
	 ...
	 * the computation. If a {@link ComputationException} was thrown from
	 * the computer, it will be propagated.</li>
	 * </ul>
	 *
	 * <p>
	 * <b>Note:</b> Callers of {@code get()} <i>must</i> ensure that the
	 * key argument is of type {@code K}. {@code Map.get()} takes
	 * {@code Object}, so the key type is not checked at compile time.
	 * Passing an object of a type other than {@code K} can result in that
	 * object being unsafely passed to the computer function as type
	 * {@code K} not to mention the unsafe key being stored in the map.
	 *
	 * <p>
	 * If {@link java.util.Map#put} is called before a computation
	...
	private static class StrategyImpl<K, V> implements Serializable,
		...
		public ReferenceEntry<K, V> copyEntry(K key,
			...
			if (valueReference == COMPUTING) {
				...
				ReferenceEntry<K, V> newEntry = newEntry(key,
						original.getHash(), newNext);
				newEntry.setValueReference(
						valueReference.copyFor(newEntry));
				return newEntry;
			}
		}
		...
		public V waitForValue(ReferenceEntry<K, V> entry)
			...
			if (valueReference == COMPUTING) {
				synchronized (entry) {
					while ((valueReference = entry
							.getValueReference()) == COMPUTING) {
						entry.wait();
					}
				}
			}
		...
		public V compute(K key, final ReferenceEntry<K, V> entry,
			...
			try {
				...
				// if computer has thrown a computation exception,
				// propagate rather
				// than wrap
				setValueReference(entry,
						new ComputationExceptionReference<K, V>(
								e.getCause()));
				throw e;
			} catch (Throwable t) {
				setValueReference(entry,
		...
		private static class Fields {
			...
			static final Field	valueStrength	= findField(
					"valueStrength");
			static final Field	expirationNanos	= findField(
					"expirationNanos");
	...
	// Safe because impl never uses a parameter or returns any non-null
	// value
	 ...
	 */
	private static class SoftEntry<K, V> extends
			FinalizableSoftReference<K> implements ReferenceEntry<K, V> {
	 ...
	 */
	private static class WeakEntry<K, V> extends
			FinalizableWeakReference<K> implements ReferenceEntry<K, V> {
