 ...
 * CustomConcurrentHashMap class itself is not extensible and does not
 * contain any methods. Use {@link Builder} to create a custom concurrent
 * hash map instance. Client libraries implement {@link Strategy}, and this
 * class provides the surrounding concurrent data structure which
 * implements {@link ConcurrentMap}. Additionally supports implementing
 * maps where {@link Map#get} atomically computes values on demand (see
 ...
 * are thread-safe, retrieval operations do <i>not</i> entail locking, and
 * there is <i>not</i> any support for locking the entire table in a way
 * that prevents all access.
 *
 * <p>
 * Retrieval operations (including {@link Map#get}) generally do not block,
 * so may overlap with update operations (including {@link Map#put} and
 * {@link Map#remove}). Retrievals reflect the results of the most recently
 * <i>completed</i> update operations holding upon their onset. For
 * aggregate operations such as {@link Map#putAll} and {@link Map#clear},
 * concurrent retrievals may reflect insertion or removal of only some
 * entries. Similarly, iterators return elements reflecting the state of
 * the hash table at some point at or since the creation of the iterator.
 * They do <i>not</i> throw
 * {@link java.util.ConcurrentModificationException}. However, iterators
 * can only be used by one thread at a time.
 *
 * <p>
 * The resulting {@link ConcurrentMap} and its views and iterators
 * implement all of the <i>optional</i> methods of the
 * {@link java.util.Map} and {@link java.util.Iterator} interfaces.
 * Partially reclaimed entries are never exposed through the views or
 * iterators.
 *
 * <p>
 ...
 * {
 * 	&#64;code
 * 	class ConcurrentHashMapStrategy<K, V> implements
 * 			CustomConcurrentHashMap.Strategy<K, V, InternalEntry<K, V>>,
 * 			Serializable {
 ...
 * 				InternalEntry<K, V> original,
 * 				InternalEntry<K, V> next) {
 * 			return new InternalEntry<K, V>(key, original.hash,
 * 					original.value, next);
 * 		}
 * 
 ...
 * 		}
 * 
 * 		public V getValue(InternalEntry<K, V> entry) {
 * 			return entry.value;
 * 		}
 * 
 ...
 * 		}
 * 
 * 		public K getKey(InternalEntry<K, V> entry) {
 * 			return entry.key;
 * 		}
 * 
 ...
 * 		}
 * 
 * 		public int getHash(InternalEntry<K, V> entry) {
 * 			return entry.hash;
 * 		}
 * 
 * 		public void setInternals(
 * 				CustomConcurrentHashMap.Internals<K, V, InternalEntry<K, V>> internals) {
 * 		} // ignored
 * 	}
 *
 ...
 * 
 * 		InternalEntry(K key, int hash, V value,
 * 				InternalEntry<K, V> next) {
 * 			this.key = key;
 * 			this.hash = hash;
 * 			this.value = value;
 ...
 * </pre>
 *
 * To create a {@link java.util.concurrent.ConcurrentMap} using the
 * strategy above:
 *
 * <pre>
 * {
...
final class CustomConcurrentHashMap {
	...
	final static class Builder {
		...
		/**
		 * Sets a custom initial capacity (defaults to 16). Resizing this
		 * or any other kind of hash table is a relatively slow operation,
		 * so, when possible, it is a good idea to provide estimates of
		...
		/**
		 * Guides the allowed concurrency among update operations. Used as
		 * a hint for internal sizing. The table is internally partitioned
		 * to try to permit the indicated number of concurrent updates
		 * without contention. Because placement in hash tables is
		 * essentially random, the actual concurrency will vary. Ideally,
		 * you should choose a value to accommodate as many threads as will
		 * ever concurrently modify the table. Using a significantly higher
		 * value than you need can waste space and time, and a
		 * significantly lower value can lead to thread contention. But
		 * overestimates and underestimates within an order of magnitude do
		 * not usually have much noticeable impact. A value of one is
		 * appropriate when it is known that only one thread will modify
		 * and all others will only read. Defaults to {@literal 16}.
		 *
		 * @throws IllegalArgumentException
		 ...
		 * @param <E>
		 *            the type of internal entry to be stored in the
		 *            returned map
		 *
		 * @throws NullPointerException
		 *             if strategy is null
		 */
		public <K, V, E> ConcurrentMap<K, V> buildMap(
		...
		/**
		 * Creates a {@link ConcurrentMap}, backed by the given strategy,
		 * that supports atomic, on-demand computation of values.
		 * {@link Map#get} returns the value corresponding to the given
		 * key, atomically computes it using the computer function passed
		 * to this builder, or waits for another thread to compute the
		 * value if necessary. Only one value will be computed for each key
		 * at a given time.
		 *
		 * <p>
		 * If an entry's value has not finished computing yet, query
		 * methods besides {@link java.util.Map#get} return immediately as
		 * if an entry doesn't exist. In other words, an entry isn't
		 * externally visible until the value's computation completes.
		 *
		 * <p>
		 ...
		 * <li>or {@link ComputationException} wrapping an exception thrown
		 * by the computation</li>
		 * </ul>
		 *
		 * <p>
		 * <b>Note:</b> Callers of {@code get()} <i>must</i> ensure that
		 * the key argument is of type {@code K}. {@code Map.get()} takes
		 * {@code Object}, so the key type is not checked at compile time.
		 * Passing an object of a type other than {@code K} can result in
		 * that object being unsafely passed to the computer function as
		 * type {@code K} not to mention the unsafe key being stored in the
		 * map.
		 *
		 * @param strategy
		 *            used to implement and manipulate the entries
	 ...
	 * implementation. Used by the framework to create new entries and
	 ...
	 * Setting values to null is not permitted, but entries may have null
	 * keys or values for various reasons. For example, the key or value
	 * may have been garbage collected or reclaimed through other means.
	 * CustomConcurrentHashMap treats entries with null keys and values as
	 * "partially reclaimed" and ignores them for the most part. Entries
	 * may enter a partially reclaimed state but they must not leave it.
	 * Partially reclaimed entries will not be copied over during table
	 * expansions, for example. Strategy implementations should proactively
	 * remove partially reclaimed entries so that {@link Map#isEmpty} and
	 ...
	 *            the type of values to be stored in the returned map
	 * @param <E>
	 *            internal entry type, not directly exposed to clients in
	 *            map views
	 */
	public interface Strategy<K, V, E> {

		/**
		 * Constructs a new entry for the given key with a pointer to the
		 * given next entry.
		 *
		 * <p>
		 * This method may return different entry implementations depending
		 * upon whether next is null or not. For example, if next is null
		 * (as will often be the case), this factory might use an entry
		 ...
		 * @param next
		 *            entry (used when implementing a hash bucket as a
		 *            linked list, for example), possibly null
		 * @return a new entry
		 */
		abstract E newEntry(K key, int hash, E next);

		/**
		 * Creates a copy of the given entry pointing to the given next
		 * entry. Copies the value and any other implementation-specific
		 * state from {@code original} to the returned entry. For example,
		 * CustomConcurrentHashMap might use this method when removing
		 * other entries or expanding the internal table.
		 *
		 * @param key
		 *            for {@code original} as well as the returned entry.
		 *            Explicitly provided here to prevent reclamation of
		 *            the key at an inopportune time in implementations
		 *            that don't otherwise keep a strong reference to the
		 *            key.
		 * @param original
		 *            entry from which the value and other
		 *            implementation-specific state should be copied
		 * @param newNext
		 *            the next entry the new entry should point to,
		 *            possibly null
		 */
		E copyEntry(K key, E original, E newNext);

		/**
		 * Sets the value of an entry using volatile semantics. Values are
		 * set synchronously on a per-entry basis.
		 *
		 * @param entry
		 *            to set the value on
		 * @param value
		...
		/**
		 * Gets the key for the given entry. This may return null (for
		 * example, if the key was reclaimed by the garbage collector).
		 *
		 * @param entry
		 *            to get key from
		...
		/**
		 * Gets the next entry relative to the given entry, the exact same
		 * entry that was provided to {@link Strategy#newEntry} when the
		 * given entry was created.
		 *
		...
		/**
		 * Returns the hash code that was passed to
		 * {@link Strategy#newEntry}) when the given entry was created.
		 */
		int getHash(E entry);

		// TODO:
		// /**
		// * Notifies the strategy that an entry has been removed from the
		// map.
		// *
	...
	public interface Internals<K, V, E> {
		...
		/**
		 * Returns the internal entry corresponding to the given key from
		 * the map.
		 *
		 * @param key
		...
		/**
		 * Removes the given entry from the map if the value of the entry
		 * in the map matches the given value.
		 *
		 * @param entry
		 *            to remove
		 * @param value
	...
	/**
	 * Extends {@link Strategy} to add support for computing values
	 * on-demand. Implementations should typically intialize the entry's
	 * value to a placeholder value in
	 * {@link #newEntry(Object, int, Object)} so that
	 * {@link #waitForValue(Object)} can tell the difference between a
	 * pre-intialized value and an in-progress computation.
	 * {@link #copyEntry(Object, Object, Object)} must detect and handle
	 * the case where an entry is copied in the middle of a computation.
	...
	public interface ComputingStrategy<K, V, E> extends Strategy<K, V, E> {
		...
		/**
		 * Computes a value for the given key and stores it in the given
		 * entry. Called as a result of {@link Map#get}. If this method
		 * throws an exception, CustomConcurrentHashMap will remove the
		 * entry and retry the computation on subsequent requests.
		 *
		 * @param entry
		 *            that was created
		 * @param computer
	...
	/**
	 * Applies a supplemental hash function to a given hash code, which
	 * defends against poor quality hash functions. This is critical when
	 * the concurrent hash map uses power-of-two length hash tables, that
	 * otherwise encounter collisions for hash codes that do not differ in
	 * lower or upper bits.
	 *
	 * @param h
	 *            hash code
	...
	static class Impl<K, V, E> extends AbstractMap<K, V>
		 ...
		 * specified by either of the constructors with arguments. MUST be
		 * a power of two <= 1<<30 to ensure that entries are indexable
		 * using ints.
		 */
		...
		/**
		 * Mask value for indexing into segments. The upper bits of a key's
		 * hash code are used to choose the segment.
		 */
		final int				segmentMask;

		/**
		 * Shift value for indexing within segments. Helps prevent entries
		 * that end up in the same segment from also ending up in the same
		 * bucket.
		 */
		...
		Segment[] newSegmentArray(int ssize) {
			...
			// inner classes of generic types apparently). Safe because
			// we're
		...
		final class Segment extends ReentrantLock {
			 ...
			 * additions are performed at the front of each bin. This makes
			 * it easy to check changes, and also fast to traverse. When
			 * nodes would otherwise be changed, new nodes are created to
			 * replace them. This works well for hash tables since the bin
			 * lists tend to be short. (The average length is less than two
			 * for the default load factor threshold.)
			 *
			 * Read operations can thus proceed without locking, but rely
			 * on selected uses of volatiles to ensure that completed write
			 * operations performed by other threads are noticed. For most
			 * purposes, the "count" field, tracking the number of
			 * elements, serves as that volatile variable ensuring
			 * visibility. This is convenient because this field needs to
			 * be read in many read operations anyway:
			 *
			 * - All (unsynchronized) read operations must first read the
			 * "count" field, and should not look at table entries if it is
			 * 0.
			 *
			 * - All (synchronized) write operations should write to the
			 * "count" field after structurally changing any bin. The
			 * operations must not take any action that could even
			 * momentarily cause a concurrent read operation to see
			 * inconsistent data. This is made easier by the nature of the
			 * read operations in Map. For example, no operation can reveal
			 * that the table has grown but the threshold has not yet been
			 * updated, so there are no atomicity requirements for this
			...
			/**
			 * Sets table to new HashEntry array. Call only while holding
			 * lock or in constructor.
			 */
			...
			public E getEntry(Object key, int hash) {
				...
				if (count != 0) { // read-volatile
					for (E e = getFirst(hash); e != null; e = s
							.getNext(e)) {
						if (s.getHash(e) != hash) {
							continue;
						}
			...
			boolean containsValue(Object value) {
				...
				if (count != 0) { // read-volatile
					...
					for (int i = 0; i < length; i++) {
						for (E e = table.get(i); e != null; e = s
								.getNext(e)) {
			...
			void expand() {
				 ...
				 * must either stay at same index, or move with a power of
				 * two offset. We eliminate unnecessary node creation by
				 * catching cases where old nodes can be reused because
				 * their next fields won't change. Statistically, at the
				 * default threshold, only about one-sixth of them need
				 * cloning when a table doubles. The nodes they replace
				 * will be garbage collectable as soon as they are no
				 * longer referenced by any reader thread that may be in
				 * the midst of traversing table right now.
				 */

				Strategy<K, V, E> s = Impl.this.strategy;
				AtomicReferenceArray<E> newTable = newEntryArray(
						oldCapacity << 1);
				threshold = (int) (newTable.length() * loadFactor);
				int newMask = newTable.length() - 1;
				for (int oldIndex = 0; oldIndex < oldCapacity; oldIndex++) {
					// We need to guarantee that any existing reads of old
					// Map can
					...
					if (head != null) {
						...
						if (next == null) {
							...
							// Reuse the consecutive sequence of nodes with
							// the same target
							// index from the end of the list. tail points
							// to the first
							...
							for (E last = next; last != null; last = s
								...
								if (newIndex != tailIndex) {
									// The index changed. We'll need to
				...
				try {
					...
					for (E e = first; e != null; e = s.getNext(e)) {
						...
						if (s.getHash(e) == hash && entryKey != null
								&& s.equalKeys(entryKey, key)) {
							V entryValue = strategy.getValue(e);
							if (value == entryValue || (value != null
									&& entryValue != null
		...
		public boolean isEmpty() {
			...
			/*
			 * We keep track of per-segment modCounts to avoid ABA problems
			 * in which an element in one segment was added and in another
			 * removed during traversal, in which case the table was never
			 * actually empty at any point. Note the similar use of
			 * modCounts in the size() and containsValue() methods, which
			 * are the only other methods also susceptible to ABA problems.
			 */
		 ...
		 *         key in this table, as determined by the {@code equals}
		 ...
		 * These mappings replace any mappings that this map had for any of
		...
		/**
		 * Returns a {@link java.util.Set} view of the keys contained in
		 * this map. The set is backed by the map, so changes to the map
		 * are reflected in the set, and vice-versa. The set supports
		 * element removal, which removes the corresponding mapping from
		 * this map, via the {@code Iterator.remove}, {@code Set.remove},
		...
		/**
		 * Returns a {@link java.util.Collection} view of the values
		 * contained in this map. The collection is backed by the map, so
		...
		/**
		 * Returns a {@link java.util.Set} view of the mappings contained
		...
		abstract class HashIterator {
			...
			/**
			 * Finds the next entry in the current chain. Returns true if
			 * an entry was found.
			 */
			boolean nextInChain() {
				Strategy<K, V, E> s = Impl.this.strategy;
				if (nextEntry != null) {
					for (nextEntry = s.getNext(
							nextEntry); nextEntry != null; nextEntry = s
									.getNext(nextEntry)) {
			...
			/**
			 * Finds the next entry in the current table. Returns true if
			 * an entry was found.
			 */
			boolean nextInTable() {
				while (nextTableIndex >= 0) {
					if ((nextEntry = currentTable
							.get(nextTableIndex--)) != null) {
			...
			/**
			 * Advances to the given entry. Returns true if the entry was
			 * valid, false if it should be skipped.
			 */
		...
		final class WriteThroughEntry extends AbstractMapEntry<K, V> {
			 ...
			 * changes, the most recent "previous" value could be different
			 * from what we return (or could even have been removed in
			 * which case the put will re-establish). We do not and cannot
			 * guarantee more.
			 */
			@Override
		...
		static class Fields {
			...
			static final Field	segmentShift	= findField(
					"segmentShift");
	...
	static class ComputingImpl<K, V, E> extends Impl<K, V, E> {
		...
		public V get(Object k) {
			 ...
			 * Findbugs can catch situations where this isn't the case.
			 *
			 * The alternative is to add an overloaded method, but the
			 * chances of a user calling get() instead of the new API and
			 * the risks inherent in adding a new API outweigh this little
			 * hole.
			 */
			@SuppressWarnings("unchecked")
			K key = (K) k;
			...
			outer: while (true) {
				...
				if (entry == null) {
					...
					segment.lock();
					try {
						// Try again--an entry could have materialized in
						// the interim.
						...
						if (entry == null) {
							...
							E first = table.get(index);
							++segment.modCount;
							entry = computingStrategy.newEntry(key, hash,
									first);
	 ...
	 * customized behavior. For example, the following creates a map that
	 * uses byte arrays as keys:
	 * 
	 * <pre>
	 *    {@code
	 *
	 ...
	 * </pre>
	 *
	 * With nothing overridden, it yields map behavior equivalent to that
	 * of {@link ConcurrentHashMap}.
	 */
